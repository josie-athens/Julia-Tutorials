...
# ╔═╡ 3fdeb57d-ff59-4398-8b55-87f533f3376a
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ 78bdc65f-c6ea-4a72-892a-8d7024bd5185
begin
	using StatsBase, Tidier, DataFrames, CSV 
	using TexTables, RCall, MLJ
end

# ╔═╡ a6ed606f-bdd3-45b2-97ec-d7182b62c1ad
include("pubh.jl");

# ╔═╡ 751c1cc4-2d07-11ee-0c10-47eacf2ca9bd
md"""
...
# ╔═╡ f0d65de3-890b-47ac-9c6f-ed5771659e5a
md"""
## 📖 Main Menu
...
# ╔═╡ 4e2a3838-729d-497b-ad09-3008028245d0
# ╠═╡ show_logs = false
R"""
...
# ╔═╡ 04d82d06-8771-43f4-a9a6-6eb3d195dc06
begin
	@rimport pubh
	@rimport readr
end

# ╔═╡ 3c32312d-7136-4156-9fd4-1837c030a2a5
md"""
# Introduction
...
# ╔═╡ ff17ed45-4b7c-4018-9aff-15eb601bac0b
md"""
!!! note
...
# ╔═╡ 0968a84e-730b-42f1-b094-8220702484ba
wcgs = readr.read_rds("data/wcgs.rds") |> rcopy; wcgs |> schema

# ╔═╡ b92488b8-0c77-4015-b2fb-e2ac0ebd50b5
kfm = readr.read_rds("data/kfm.rds") |> rcopy; kfm |> schema

# ╔═╡ 2148747f-80bd-435f-8cff-62b1a54972fe
md"""
# Contingency Tables
...
# ╔═╡ 38477e64-4523-4e6f-a1c9-35d8eb2f195b
tabulate(wcgs, :chd, :dib_pat)

# ╔═╡ 39e75aa8-4973-4489-9426-350908f79f64
md"We can print the ascii table, to display a better table."

# ╔═╡ 68e3c99e-136c-4c97-9f6f-30190f2ff2ba
tabulate(wcgs, :chd, :dib_pat) |> to_ascii |> print

# ╔═╡ 76f7c92f-e213-4edb-980a-0f46776aa484
md"With `cross_table` from `R`, we can show both frequency and proportion."

# ╔═╡ 0126bff2-ade3-41c0-b286-d9fdd236a6ce
@chain wcgs begin
	select(:chd, :dib_pat)
	pubh.cross_tbl(by="dib_pat")
	print
end

# ╔═╡ 0b5d8586-a228-4f3b-9ee5-84b0b28851af
md"""
!!! note
...
# ╔═╡ 6a6ebd37-910b-4775-bfd0-97bac82e3d57
R"""
...
# ╔═╡ 02a627b2-f780-4df9-b5a2-1351bc8b5372
md"""
!!! note
...
# ╔═╡ 814ea306-0a8e-425a-8876-2c41b9f33815
R"""
wcgs %>%
mutate(
	chd = relevel(chd, ref = "CHD"),
...
# ╔═╡ 43d0e8d1-f724-4410-851a-5051b1c822a9
md"""
...
# ╔═╡ d53c1394-651c-493a-a42f-b016318e6d22
md"""
!!! warning \"Exercise\"
...
# ╔═╡ da5562b5-f076-49fa-801d-ef22b5ca8e31
md"""
In `R` mode:
...
# ╔═╡ af507e3f-17f3-4bc9-9487-8a477481f65a
R"""
wcgs %>%
mutate(
	chd = relevel(chd, ref = "CHD")
...
# ╔═╡ a2f7f8a1-23e7-4532-bfe6-a6278dcbc904
md"""
Using `tabulate`:
...
# ╔═╡ 7cb2e8a7-b183-45d1-97ae-bc46d0ba5e8b
tabulate(wcgs, :chd, :dib_pat)

# ╔═╡ 7637a68d-60eb-4dc5-a190-75d6e779b1f1
md"""
!!! tip \"Question\"
...
# ╔═╡ d046e065-24a7-4582-9ac3-2cf39d03ada2
md"""
...
# ╔═╡ d41d0767-30a9-47c7-8991-39993ff13ae4
md"""
...
# ╔═╡ bfba9716-d2a1-48d0-82dd-c87632ef5530
R"""
wcgs %>% 
...
# ╔═╡ b5f834a5-2f2b-4939-810e-35ac21aecc86
md"""
!!! note \"Notes for R code\"
...
# ╔═╡ a50b823c-bfee-40e7-a021-19958fc758ac
md"""
...
# ╔═╡ 1cf3b152-e472-4df3-ae4c-4b402b31599b
@chain kfm begin
	select(Not(1, 3))
	describe(:min, :max, :mean, :median, :std)
end

# ╔═╡ 665fd490-3fd6-4e34-a19f-2d7d4eba4999
md"""
!!! tip
...
# ╔═╡ e9e69e8d-ba5f-4720-b881-2569b763a020
md"We can also use `summarize`."

# ╔═╡ f71af3c1-91a6-46c7-9dac-9826c180b86e
@chain kfm begin
	select(Not(1, 3))
	summarize
	to_ascii
...
# ╔═╡ 36a6544b-fdef-4bef-bb94-1c9b145a539a
md"And with more detailed summaries:"

# ╔═╡ 929966ff-b1de-4b34-8157-1de2857b4b41
@chain kfm begin
	select(Not(1, 3))
	summarize(detail=true)
end

# ╔═╡ 475606f3-828f-4f0c-bf12-313e20745fb4
labs = [
...
# ╔═╡ c719ba36-0a48-4eef-aaa7-3d827919201a
@chain kfm begin
	select(Not(1, 3, 5))
	estat(labs)
end

# ╔═╡ b9ad3466-d78d-435d-aa1e-779248029184
md"""
!!! warning
...
# ╔═╡ f363b78e-c7e0-44bf-9a37-090fd16d9b77
@chain kfm begin
	select(Not([:no, :sex, :ml_suppl]))
	estat(labs)
end

# ╔═╡ 22a9ff87-c9d5-4840-b347-0bf3e4199c86
md"""
## Stratification
"""

# ╔═╡ a1bd0cb3-0b3d-4807-a61e-1dd5f381af52
kfm |> names

# ╔═╡ ed570afb-d0e5-4502-9dcc-e43a0b5ffaec
import GLM.@formula

# ╔═╡ 0d9c66d6-2be2-4669-90f8-57d103292ad5
pubh.estat(@formula(weight ~ sex), data=kfm) |> rcopy

# ╔═╡ 14d018f8-53e0-41c4-b71e-24d706248767
md"Descriptive statistics of `kfm` stratified by sex:"

# ╔═╡ ec3730c9-b8d5-4307-b28d-207d942a8408
R"""
...
# ╔═╡ 0f83da5c-9d29-4f71-bb12-6a1f19100b30
R"""
wcgs %>%
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "cbdf56a473d7edbe4ca44774ac80789aa8da9091"
