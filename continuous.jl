...
qq_plot(
	bernard.temp0, ylabel="Baseline Temperature (°C)"
)
...
```julia
qq_plot(
	bern.temp_change, ylabel="Temperature Change (°C)"
)
```
""" |> hint
...
let
	...
	p1 = qq_plot(
		ctl.ratio,
		ylabel="CD4⁺ / CD8⁺", title="Non-Hodgkin"
	)

	p2 = qq_plot(
		tx.ratio,
		ylabel="", title="Hodgkin"
	)
...
strip_error(
	...
	xlabel="Smoking status", ylabel="Birth weight (kg)"
)
...
@df smoke_eff scatter(
	...
	yerr=:err, leg=false, 
	xrot=20, ylim=(2.5, 4), mc=jama[1], ms=2
)
...
qq_plot(air.log_oz, ylabel="log (Ozone)")
...
@df air_eff scatter(
	:Month, :log_oz,
	ylabel="log (Ozone)",
	yerr=:err, leg=false, mc=jama[1], ms=2
)
...
strip_error(
	fent, :group, :pain,
	xlabel="Cohort", ylabel="Pain reduction"
)
```
""" |> hint
...
manifest_format = "2.0"
project_hash = "2f726f998e01836bb490da19fc67992c89564ffd"
