...
# ╔═╡ 6948075b-20f9-4280-809e-dffc721735f9
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ d5de63c2-6530-4ed4-8338-fff0fdcedfe6
begin
	using StatsBase, Tidier, DataFrames, CSV
	using TexTables, RCall, RDatasets
	using MLJ
end

# ╔═╡ c2643cb5-3d90-4a2a-9a0d-12c88c2620fa
include("pubh.jl");

# ╔═╡ 1be41550-1491-11ee-1a75-d3edf875039a
md"""
...
# ╔═╡ 93e36d9c-4873-4e45-967b-cd991b704360
md"""
## 📖 Main Menu
...
# ╔═╡ da9cb399-93f1-4d4a-96b5-821edc5d2675
@rimport readr

# ╔═╡ 565f574c-eb26-4a15-a3ba-1e56536a5441
md"""
# Data Manipulation
"""

# ╔═╡ 69557493-7a50-4a9f-a5c6-383d5bddc6eb
md"""
...
As in `Pluto` each variable is defined only once, we construct and modify the data set in the same block of code.
"""

# ╔═╡ 27dc3bfd-2f63-4475-a33f-30af38fa30e1
begin
	uric_down = DataFrame(
	...
	uric_down = @chain uric_down begin
  		@mutate(
    		downs = if_else(downs == 0, "No", "Yes"),
    		sex = if_else(sex == 0, "Male", "Female")
  		)
	end
end;

# ╔═╡ 078f79b2-7037-4156-b6ee-88172a544316
uric_down |> head

# ╔═╡ 24280dce-3ef0-4bd4-a7e9-4a1cb2c1f91f
coerce!(
  	uric_down,
  	:downs => Multiclass,
  	:sex => Multiclass
...
# ╔═╡ 0d597594-4582-4fb7-b689-a21cb2a5f368
uric_down |> schema

# ╔═╡ 5e8c4132-4427-45b4-b627-c9bfdfdcc8bb
md"""
## Imput from CSV
"""

# ╔═╡ aba2f79a-0fd5-4e04-8398-efac222ffa7e
wcgs2 = DataFrame(CSV.File("data/wcgs.csv")); wcgs2 |> schema

# ╔═╡ f8aeebf5-9890-4b9f-a5bb-154ed3840c0b
md"Dimensions:"

# ╔═╡ 23c1651b-7768-43de-a4db-945cf29555c2
wcgs2 |> size

# ╔═╡ b6aaf6af-e1f3-458a-af3c-cdaa54071629
md"Names of columns (variables):"

# ╔═╡ a98b9d31-1b7e-49f4-b953-fe44d37f7a9c
wcgs2 |> names

# ╔═╡ 94b4fecb-aff7-499a-90bb-fd5f1f0f51b8
md"## Import from rds"

# ╔═╡ 8d98b149-995a-40b1-bbe4-ba776325a8e2
kfm = readr.read_rds("data/kfm.rds") |> rcopy; kfm |> schema

# ╔═╡ f3acd5f3-5a8d-4fb4-b720-20f556ffe337
kfm |> head

# ╔═╡ 97ee3d0d-8d19-4aa4-92da-92dbff50812c
md"## Changing names of variables"

# ╔═╡ 8e9d5764-8de3-49ce-a345-7be50a1fa5cb
md"""
For Pluto notebooks, variables are defined only once, hence I am showing the whole data wrangling of the dataset and explain each section on corresponding subsections.
"""

# ╔═╡ 257911a3-cfb4-4dfe-bb4a-c012732db82d
wcgs = @chain wcgs2 begin
	@rename(
  		age = age0,
  		height = height0,
  		weight = weight0,
	...
	@mutate(
    	chd = if_else(chd == 0, "No CHD", "CHD"),
    	arcus = if_else(arcus == 0, "Absent", "Present"),
    	beh_pat = case_when(
      		beh_pat == 1 => "A1",
      		beh_pat == 2 => "A2",
      		beh_pat == 3 => "B1",
    	...
    	dib_pat = if_else(dib_pat == 0, "B", "A"),
    	type_chd = case_when(
      		type_chd == 0 => "No CHD",
      		type_chd == 1 => "MI or SD",
      		type_chd == 2 => "Angina",
      		type_chd == 3 => "Silent MI"
    	),
		smoker = if_else(ncigs == 0, "Non-Smoker", "Smoker"),
		height = height * 2.54,
		weight = weight * 0.4536
  )
end;

# ╔═╡ c3a466dd-a684-47f6-981b-feb24b65e0b4
wcgs[1:5, 2:7]

# ╔═╡ dc54b8cb-424f-4bed-9794-a64ca557f2d0
md"""
!!! note

	In `@rename` the new name is at the left of the `=` (equals) while the original name is at the right side, like any other assignment.
"""

# ╔═╡ 32f90e13-f651-4460-b461-b5ebc50aca8c
md"## Creating Factors"

# ╔═╡ 34f6c405-e455-4683-a993-251f210f4b2e
md"""
!!! note

	First we mutate our categorical variables from Int64 to String with `@mutate` (see long code above).

	- For variables with only two levels, we used: `if_else`.
	- For variables with three or more levels, we used: `case_when`.
"""

# ╔═╡ bdee8957-8b2e-4c17-bffd-7fcdf2b18c33
md"""
!!! note
...
# ╔═╡ f6e0adeb-1cb8-4c2d-b76b-247b22a6e67f
coerce!(
    wcgs,
    :chd => Multiclass,
    :arcus => Multiclass,
...
# ╔═╡ 2792b4a6-999e-49da-af03-1542320e3d2c
@select(wcgs, chd, arcus, beh_pat, dib_pat, type_chd) |>
schema

# ╔═╡ 4e149173-8d17-4675-974e-2d18b5f920e0
levels(wcgs.chd)

# ╔═╡ 2f54057d-da1b-4924-997f-bda731ccea09
md"""
## Transforming to a binary variable

One of our variables is a count and stores the number of smoked cigarettes/day. We defined a new variable `Smoker` in which, everyone who smokes one or more cigarette/day was smoker. We generated the binary variable using `if_else` and then converted it to a factor with `coerce!`.
"""

# ╔═╡ db10aadf-d42f-4330-81fa-6a8f57a290ed
md"""
## Simple numeric transformations

We also, prefer units in the metric system. We converted `height` from inches to centimetres and `weight` from pounds to kg.
"""

# ╔═╡ 172af9b5-594a-4a15-9dda-c9afacaeea42
md"""
## Centring
...
# ╔═╡ e7dfc660-fe17-4b33-bd9f-7aea7637b5cf
@chain wcgs begin
  @mutate(height_cent = height - mean(height))
  @select(height, height_cent)
  estat
end

# ╔═╡ 6692e05a-ae76-4161-bf15-74b00be90daa
md"""
# Indexing and subsets

## Subsets

Let’s said that we are only interested in subjects who are smokers. If that is the case, we can create a new data frame. We can use either, the `subset` function from `DataFrameMacros`.
"""

# ╔═╡ 755d222b-ae90-45a0-b70a-7123e75f68e7
@filter(wcgs, smoker == "Smoker") |> nrow

# ╔═╡ 2b7f5dd0-d071-4bda-83f5-726144d3a147
smokers = @filter(wcgs, smoker == "Smoker");

# ╔═╡ 9c1cde7e-2263-4f1c-ba5e-8cde7d814d4a
smokers[1:5, 2:6]

# ╔═╡ 436f3133-f16d-4977-82fc-8211b30e57f4
md"Let's check for the number of observations:"

# ╔═╡ 61782b63-1dbe-491c-9218-544109cd3051
wcgs |> nrow

# ╔═╡ fba204c6-fec8-4e9c-b76b-ea3dbb17ef34
smokers |> nrow

# ╔═╡ a9000bee-5340-4dde-9953-cfa3355da076
md"We can access one of those columns easily using `.colname`, this returns a vector that you can access like any Julia vector:"

# ╔═╡ 60e7cbda-b4e6-4678-8f15-499f03d93ae7
wcgs.chd[1:5]

# ╔═╡ bbf4ef92-f5e6-49d7-a042-74f329e6a338
md"Another option is to use `select` and for negative indexing, `Not`"

# ╔═╡ 5e5db696-e193-4045-a085-779c904fcd6e
@select(wcgs, ncigs, smoker) |> head

# ╔═╡ 78c70557-fe35-49df-931b-4d7a4fa15443
md"""
!!! tip
...
# ╔═╡ 4b53f37b-d700-406d-8f43-093f4d5d386f
@chain smokers begin
  select(Not([:id, :type_chd, :ncigs, :beh_pat]))
  size()
end

# ╔═╡ 088205d6-9a93-4d6e-8c01-41c312da6849
md"""
!!! note
...
# ╔═╡ 29f8dfdb-9e48-4ac6-865b-a50bce946471
md"""
!!! important

	`DataFrameMacros` use the `{}` expression to include several columns. This is also needed for negative indexing when using `@select`.	
"""

# ╔═╡ 2c0af612-897f-4b4d-9a5b-43121244ed61
@chain smokers begin
  select(Not([:id, :type_chd, :ncigs, :beh_pat]))
  size()
end

# ╔═╡ c52803cd-5f57-4fa5-ac9d-a80dd8d007f1
md"""
!!! note
...
# ╔═╡ d8968ed1-bba0-4b59-a9e8-2094eb3090ee
md"""
## Queries
"""

# ╔═╡ c85d6004-c474-4bbd-99fd-7f95dbe4bac9
md"""
!!! tip \"Example\"
...
# ╔═╡ bba401b7-2f39-41e7-9e44-dc9fe48c505f
@chain smokers begin
  @filter(weight >= 100)
  @summarise(n = n())
end

# ╔═╡ 6f1f2a4c-5f17-4308-9de8-92889c30eff4
md"""
Or more straightforward:
"""

# ╔═╡ 76e3565a-e71e-48ec-b072-1b6636200dd1
@filter(smokers, weight >= 100) |> nrow

# ╔═╡ 4f294442-0285-46f8-b3fd-1dd1199f272d
md"""
...
# ╔═╡ 2b36c5bc-ff04-436e-b5b7-f33d652bdb64
@filter(wcgs, smoker == "Smoker", weight >= 100) |> nrow

# ╔═╡ 2a17da69-4eec-4514-a082-c86cb5a59d4d
md"""
!!! danger \"Exercise\"
...
# ╔═╡ c5e17999-d47d-41e9-802e-2a3caaa802f2
@filter(smokers, beh_pat == "A2", type_chd in ("Angina", "Silent MI")) |> nrow

# ╔═╡ 014e8ad1-c186-412f-a1fb-1556734339f2
md"""
!!! tip
...
# ╔═╡ 03469dbb-f29c-40d0-a63b-2277a6ef2014
@filter(smokers, beh_pat == "A2", type_chd in ("Angina", "Silent MI")) |> nrow

# ╔═╡ 7c8b716b-51b0-4d34-a1e3-2c5ffa6d5e61
md"""
!!! tip \"Example\"
...
# ╔═╡ a6e158da-ff6e-48ca-b379-a7b764ba18d7
wcgs.weight |> findmax

# ╔═╡ a6a7f38f-26b6-4298-bcc6-da564fc6ccd2
wcgs[1090, :]

# ╔═╡ 58547b2d-0537-44b9-8d24-146ef9d0067d
wcgs[findmax(wcgs.weight)[2], :]

# ╔═╡ d2209481-27e7-4586-97c7-f7f8aa8d68d4
md"""
...
# ╔═╡ a31e958b-07f4-48ac-9618-55639411d334
kfm_mat = Matrix(
	@chain kfm begin
		select(Not(1, 3, 5))
	end
...
# ╔═╡ ef02a052-eb2a-4073-9e2c-0401835b0e65
@show(size(kfm))

# ╔═╡ cc7d49c2-3293-4a48-94ab-7a3f20a8737b
@show(size(kfm_mat))

# ╔═╡ 2396da27-f54b-498b-92ec-74f16f1e7484
kfm_mat[1:5, :]

# ╔═╡ 3fd18e1e-6929-4fa6-a559-8893273c2954
md"## Missing values"

# ╔═╡ aa469bad-7a08-4ee6-a3cf-fddf66abb720
mao = dataset("gap", "mao"); mao |> schema

# ╔═╡ 13cdd6e8-7917-4fbf-afe5-59142c32f216
mao[1:7, 2:6]

# ╔═╡ 1bb6f3b7-79b1-4477-9ddb-0a5dd3f2a096
describe(mao, :nmissing)

# ╔═╡ a98cd572-7102-4c55-803d-8bfa431b30a5
md"Lots of missing values...
If we wanted to compute simple functions on columns, they  may just return `missing`:"

# ╔═╡ 1193b8f1-0668-47f7-b2ea-28ff3c6c7790
std(mao.Age)

# ╔═╡ 9e2e0206-e1a9-48c8-b198-af033cc72637
md"Some functions remove missing though:"

# ╔═╡ 40626aba-a083-431a-b5a2-b4597f2dfef7
@chain mao begin
	select(:Age)
	describe(:min, :max, :mean, :median, :std, rel_dis => :cv)
end

# ╔═╡ ada4550a-c489-4e8f-9d87-ce00356530be
md"We can use `skipmissing` or `dropmissing` to counter this  easily:"

# ╔═╡ 46b68ad1-8437-4aad-8a0b-2ba4723d6f35
std(skipmissing(mao.Age)) |> r3

# ╔═╡ af650f4d-4b79-4589-a2f9-90f28fd6383c
@chain mao |> dropmissing begin
	@select(Age)
	estat
end

# ╔═╡ 74c69e7a-c680-4d9c-8875-669ede23d641
md"""
# Group manipulations
## Split-Apply-Combine
"""

# ╔═╡ 64b4fad3-e97e-4e0f-9837-96d7b693feaf
iris = rcopy(R"datasets::iris"); iris |> schema

# ╔═╡ e3df04e3-dfb9-4fb1-9f18-70b3c404adb7
md"""
...
# ╔═╡ 5faba174-f26b-49ab-bd74-82199f30ac84
@select(iris, Species) |> unique

# ╔═╡ 0557f7d8-2281-44cf-b0e0-3a3b388c3a1b
md"We can form views for each of these:"

# ╔═╡ 4ffd3856-3c47-451f-ab6a-b77c2fc2150d
gdf = @group_by(iris, Species);

# ╔═╡ 993e3716-69fc-46e3-ae20-3d8c462360ca
begin
	subdf_setosa = gdf[1]
	describe(subdf_setosa, :mean, :median, :std, rel_dis => :cv)
end

# ╔═╡ a87e2ce8-4b53-43b9-93bb-e7af1d057d77
md"Or using pipes, without the need of creating subsets:"

# ╔═╡ 971aed49-a337-4676-a8cc-5c2ff638e583
@chain iris begin
...
# ╔═╡ 08f92cb6-fc6a-4771-bc2d-962de867a36c
md"""
!!! tip
...
# ╔═╡ b53b6b68-1129-48c6-bd71-da7eba872188
md"Let’s change the scale, for all continuous variables, as an example."

# ╔═╡ b1f03eef-0c59-423c-9a78-7b87b23c0b9b
@chain iris begin
  @filter(Species == "setosa")
  @select(
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "2d5e82b8fcfe11d7426e5c7ce5125c5fec0ce042"
