...
begin
	...
	using TexTables, RCall, RDatasets, CategoricalArrays
	using StatsPlots, MLJ
end
...
# ╔═╡ 89f0fcf8-5253-4b9e-9a0c-d5c6d863dde7
uric_down = DataFrame(
...
# ╔═╡ 622881ea-4dee-4af2-b5b1-02ff877737e1
md"""
## Generating Factors
...
# ╔═╡ f50db027-0c71-4c50-8d06-b9bf624d4b92
@transform!(
  uric_down,
...
# ╔═╡ e6b457e0-066d-43a1-8c3b-e9c1efdd763a
md"""
!!! note
...
wcgs = DataFrame(CSV.File("data/wcgs.csv")); wcgs |> schema
...
# ╔═╡ 3f3bcc2a-86bc-4041-a0a6-fe4134b7c241
DataFrames.rename!(
    wcgs,
    :age0 => :age,
    :height0 => :height,
...
# ╔═╡ db335d1c-3b09-4927-a7fe-45fdd98b38d9
begin
	@transform!(
  		wcgs,
...
One of our variables is a count and stores the number of smoked cigarettes/day. We defined a new variable `Smoker` in which, everyone who smokes one or more cigarette/day was smoker. One of the easiest ways to create binary variables is to use a conditional statement.
"""

# ╔═╡ 0fce6a1e-fc03-487d-82a9-9903f15f583e
begin
	@transform!(
  		wcgs,
...
# ╔═╡ 39cdbaa6-faeb-4d5e-8833-9df197abc90c
@transform!(
  	wcgs,
  	:height = :height * 2.54,
...
# ╔═╡ ab9ca91f-2060-4785-950a-a06541823ae1
estat(
	@select(
...
Let’s said that we are only interested in subjects who are smokers. If that is the case, we can create a new data frame. We can use the `subset` function from `DataFrameMacros`.
"""
...
# ╔═╡ 848abb53-bf8e-421d-9086-ebf7e1fff6e0
@select(
	smokers, {Not([:id, :type_chd, :ncigs, :beh_pat])}
) |> size
...
# ╔═╡ 3898e001-8d79-4cda-93f8-9f464826912c
@select(smokers, {Not(:id, :type_chd, :ncigs, :beh_pat, :smoker)}) |> head
...
@subset(smokers, :weight >= 100) |> nrow
...
# ╔═╡ 1d6a2dcc-9d08-4149-b0e2-1ecdff4c3b2d
@subset(smokers, :beh_pat == "A2", (:type_chd == "Angina" || :type_chd == "Silent MI")) |> nrow
...
# ╔═╡ 7ec850be-efb6-42f9-adc2-b7c33170bb0f
kfm_mat 
...
# ╔═╡ 1a1cafd4-21b4-402e-bb03-b59c3aa7c163
describe(
	@select(mao, :Age),
...
# ╔═╡ 059595a3-f54d-4154-83c1-f1656218971c
estat(
	@select(mao |> dropmissing, :Age)
)
...
# ╔═╡ 5012b831-f5df-4ffe-b3d8-4883b7dc07a7
estat(
	@select(
...
# ╔═╡ aa8e746e-494a-4ac6-9627-c26be4d9704e
estat(
	@select(
		@subset(iris, :Species .== "setosa"),
		{1:4} * 10
...
manifest_format = "2.0"
project_hash = "1f3ffce0fdfcc19b43edb4004e9031db1b256a9d"
