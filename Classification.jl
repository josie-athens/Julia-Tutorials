...
# ╔═╡ 195c738c-8ee6-4e0d-9c58-142ce3aac4fc
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ 39be8707-bcde-46f0-9836-8dac6bfcc2a9
begin
	using StatsBase, DataFrames, DataFrameMacros, MLJ
	using RCall, TexTables, CategoricalArrays
	using StatsPlots, TreeRecipe, CausalityTools
	using MLJ: schema
end

# ╔═╡ 05f6d7e5-405b-4a79-8779-272952c865d7
using StableRNGs

# ╔═╡ a56fd9af-8bcd-4e83-bc68-1d81b7d3586d
using MLJDecisionTreeInterface

# ╔═╡ efe9dc5b-20ac-4bba-bb86-004656837e8d
using MLJMultivariateStatsInterface

# ╔═╡ 0a16b1bc-0c35-44b1-a60a-3f2e45446e59
include("pubh.jl");

# ╔═╡ 3551c150-0fbd-11ee-14de-35c73ffc8343
md"""
...
# ╔═╡ 67eb55eb-0362-4f9c-951b-aecb5aebab08
md"""
# [📖 Main Menu](index.html)
"""

# ╔═╡ 40711b79-842b-4a3f-9006-d6e03be27309
begin
	import DecisionTree
	DT = DecisionTree
end;

# ╔═╡ 2686a8e7-c5cb-4857-ae75-8b71ba177b04
md"""
# Data
...
# ╔═╡ e102280b-af4e-479f-8c3b-008efcccca94
iris = rcopy(R"datasets::iris"); iris |> schema

# ╔═╡ 8816654d-169f-4778-b9c6-1bc81859fadb
md"Then we `unpack` the data to define `response` and `explanatory` variables:"

# ╔═╡ b9ed83bc-b0f5-4667-99aa-3b58d78cc4dc
y, X = unpack(iris, ==(:Species)); 

# ╔═╡ 1a959d18-2093-4b94-a71b-f553773d3fcd
md"## Data splitting"

# ╔═╡ a03d5091-53e7-4ec5-9623-5459fd741be8
begin
	rng = StableRNG(566)
...
# ╔═╡ 52f7c795-9d2b-4633-ad9e-43a9534e7c04
md"""
# Pruned Tree Classifier
## Model
"""

# ╔═╡ 59571e9a-5a50-4a2b-bef4-5809dd135b19
DecisionTreeClassifier = @load DecisionTreeClassifier pkg=DecisionTree verbosity=0

# ╔═╡ e954c5b4-09c9-493f-841e-67d9a4291d36
pruned_model = DecisionTreeClassifier(max_depth=2)

# ╔═╡ 8f232a02-0199-4cab-b37a-1b1aba0e85c4
pruned_mach = machine(pruned_model, X, y)

# ╔═╡ fec5ef62-e8bf-40fd-808e-375d29fd0fed
fit!(pruned_mach, rows=train)

# ╔═╡ 40a850b2-7c30-497f-b7fe-c54a42b53404
fitted_params(pruned_mach).tree

# ╔═╡ 6a93bf31-bb72-4e60-b436-a9e062f21bab
plot(fitted_params(pruned_mach).tree)

# ╔═╡ b7b64a17-ee64-4e98-9e99-4acde3be4105
pruned_mach |> feature_importances

# ╔═╡ c734c289-5923-4fc8-a2f6-945a3118c926
md"## Predictions"

# ╔═╡ b24d3f64-4b85-4f06-9b7d-671efea7262d
begin
	pruned_ŷ = MLJ.predict(pruned_mach, rows=test)
	pruned_ȳ = predict_mode(pruned_mach, rows=test)
end;

# ╔═╡ 0247b9d3-3cfa-40a7-8fe8-894835db428e
md"## Performance"

# ╔═╡ bab222ef-2d85-408f-b863-a3cde1775d71
cross_entropy(pruned_ŷ, y[test]) |> mean |> r3

# ╔═╡ d574f895-88e5-44c8-b8ce-2aae9a500bb6
misclassification_rate(pruned_ȳ, y[test]) |> r3

# ╔═╡ 0609b49b-2413-46d2-81b7-2aa190099f8e
kappa(pruned_ȳ, y[test]) |> r3

# ╔═╡ ad88da6f-9061-433f-8761-507c23f809ce
accuracy(pruned_ȳ, y[test]) |> r3

# ╔═╡ b9b97753-f2f2-4752-8d79-a4f1e6826eda
mutualinfo(
    MIShannon(), Contingency(),
    pruned_ȳ, y[test]
) |> r3

# ╔═╡ d645a035-cdc7-4b4a-bcda-ab03e5ce3c9a
mutualinfo(
    MITsallisFuruichi(q=1.5), Contingency(),
    pruned_ȳ, y[test]
) |> r3

# ╔═╡ a239eea0-fc58-4334-9a69-70e688c5c982
md"""
# Decision Tree Classifier
## Model
"""

# ╔═╡ eff988e3-c03b-429f-a2db-a905ace43c99
dt_model = DecisionTreeClassifier(merge_purity_threshold=0.9)

# ╔═╡ ffa688a6-cc01-46ab-a913-e127e6e55172
dt_mach = machine(dt_model, X, y)

# ╔═╡ 548c38bc-77dd-4315-96df-a8625a04467f
fit!(dt_mach, rows=train)

# ╔═╡ 2d162c73-428f-4cc0-bea6-83822374a220
fitted_params(dt_mach).tree

# ╔═╡ 722cc9f0-7dee-41c1-9510-08df628e2205
plot(fitted_params(dt_mach).tree)

# ╔═╡ 2be0b08d-e6c5-4b27-86f6-3ba81292a887
md"## Predictions"

# ╔═╡ d7a662f8-8783-4ea5-863d-9bc1d727d1b6
begin
	dt_ŷ = MLJ.predict(dt_mach, rows=test);
	dt_ȳ = predict_mode(dt_mach, rows=test);
end;

# ╔═╡ 1fcca4dc-8cc9-4f48-8e9c-ef8be815eea5
md"## Performance"

# ╔═╡ a7f9138a-070b-4f12-9f59-5396bf63f6de
cross_entropy(dt_ŷ, y[test]) |> mean |> r3

# ╔═╡ 229eed4b-f3d8-4004-b0b2-bd97893d5fa8
misclassification_rate(dt_ȳ, y[test]) |> r3

# ╔═╡ dda75776-3179-4337-a1d0-956419ae2d05
accuracy(dt_ȳ, y[test]) |> r3

# ╔═╡ d33d12ed-841b-4733-945c-a184524d2971
kappa(dt_ȳ, y[test]) |> r3

# ╔═╡ 849de3d6-b777-4bc3-9fe3-a23261531592
mutualinfo(
    MIShannon(), Contingency(),
    dt_ȳ, y[test]
) |> r3

# ╔═╡ ab12cc90-dee7-4b40-b93a-401c3437666f
mutualinfo(
    MITsallisFuruichi(q=1.5), Contingency(),
    dt_ȳ, y[test]
) |> r3

# ╔═╡ 65d83fe2-9800-4d56-88da-3eb95b0089ba
md"""
# Random Forests
## Model
"""

# ╔═╡ 288e2597-d211-4810-a318-808c6bbc9844
Forest = @load RandomForestClassifier pkg=DecisionTree verbosity=0

# ╔═╡ c09bbd67-deeb-48bc-ab92-528832527dae
md"Following the example from `DecisionTree`, we use particular parameters for the model."

# ╔═╡ 13a52d53-7c99-4c3e-b41c-bf132f609284
forest = Forest(
  	n_subfeatures = 2,
  	n_trees = 10,
...
# ╔═╡ 2821820e-133f-490b-8d5a-5e50ac7ad741
forest_mach = machine(forest, X, y)

# ╔═╡ f233a414-1a9b-46d1-b49e-f3c4b365dc69
fit!(forest_mach, rows=train)

# ╔═╡ 3e77cdd7-a7f2-445c-a0f4-f23ce8caef26
fitted_params(forest_mach)

# ╔═╡ 3f251a78-ab5c-4f32-b0dd-4fc3e4e019e1
forest_mach |> feature_importances

# ╔═╡ e6b1a9c1-567f-47a9-ab02-fe99047076b2
md"## Predictions"

# ╔═╡ 78a15ae8-bc57-4af6-966c-be1e43d74416
begin
	forest_ŷ = MLJ.predict(forest_mach, rows=test);
	forest_ȳ = predict_mode(forest_mach, rows=test);
end;

# ╔═╡ a5fe1057-4966-47ea-9ba3-11b0a5f5f3bb
md"## Performance"

# ╔═╡ e3dd582d-ec87-4ab0-a285-0d1e47f500ce
cross_entropy(forest_ŷ, y[test]) |> mean |> r3

# ╔═╡ c0cb04f8-0eaf-4c6b-898f-3a41b65157be
misclassification_rate(forest_ȳ, y[test]) |> r3

# ╔═╡ d005065b-54db-46ff-8548-8fd2067566a2
accuracy(forest_ȳ, y[test]) |> r3

# ╔═╡ 7b62e87e-0016-41f4-a1c2-c2a521ab76de
kappa(forest_ȳ, y[test]) |> r3

# ╔═╡ 03c38f73-6770-4335-a312-ad30c147dc9b
mutualinfo(
    MIShannon(), Contingency(),
    forest_ȳ, y[test]
) |> r3

# ╔═╡ 05bcbfe3-1fc6-4c95-b17c-255e9fba8bf2
mutualinfo(
    MITsallisFuruichi(q=1.5), Contingency(),
    forest_ȳ, y[test]
) |> r3

# ╔═╡ 5ab76ed3-7969-4224-b67e-63130bf3ff58
md"""
# Linear Discriminant Analysis
## Model
"""

# ╔═╡ 9e2e6c3f-ac83-4bde-8b46-81545035bab3
LDA = @load LDA pkg=MultivariateStats verbosity=0

# ╔═╡ aedd6eba-c199-43f3-ab85-de1489ba438e
lda_model = LDA()

# ╔═╡ 7c86ccb6-aa01-41aa-899e-e876b178272c
lda_mach = machine(lda_model, X, y)

# ╔═╡ 57748247-6d38-4100-bc24-12cd26920765
fit!(lda_mach, rows=train)

# ╔═╡ 64298c6a-669b-4926-b187-c76452c7c97f
fitted_params(lda_mach).projection_matrix |> size

# ╔═╡ 0bba2574-92b4-440d-a895-7ecd7ce30d23
md"## Predictions"

# ╔═╡ cf0d6816-f8d2-4e5b-9039-53a713493888
begin
	lda_ŷ = MLJ.predict(lda_mach, rows=test);
	lda_ȳ = predict_mode(lda_mach, rows=test);
end;

# ╔═╡ f18dfa15-d85d-4eba-a75c-b1d669914b79
md"## Visualisation"

# ╔═╡ 369f166f-08bf-44c1-aa57-b94e589b8435
lda_x = MLJ.transform(lda_mach); lda_x |> head

# ╔═╡ 8881632b-232e-4426-a890-292ea90c169d
lda_df = DataFrame(
	PC1 = lda_x.x1,
	PC2 = lda_x.x2,
	Species = string.(y)
...
# ╔═╡ c02d7086-7938-44b3-a660-e539f088beb0
@df lda_df scatter(
	:PC1, :PC2, group=:Species,
	xlabel="PC₁", ylabel="PC₂",
	color_palette=jama, ms=3,
	xlim=(-1, 1.5)
)

# ╔═╡ 5cd2a77c-32fc-4a62-b583-5df8e8b6328e
md"## Performance"

# ╔═╡ 81445d95-5668-49af-b517-92f0102bbfba
cross_entropy(lda_ŷ, y[test]) |> mean |> r3

# ╔═╡ 7fae5052-f9e4-493a-8245-0e645144ad3f
misclassification_rate(lda_ȳ, y[test]) |> r3

# ╔═╡ 3e9a39f2-e5c8-41d3-b86c-c244eb2bb226
accuracy(lda_ȳ, y[test]) |> r3

# ╔═╡ 3c539884-e01b-4ffb-ab5d-35a1a6c0725d
kappa(lda_ȳ, y[test]) |> r3

# ╔═╡ d3a09fb3-7f73-43d2-9849-e387c6e908eb
mutualinfo(
    MIShannon(), Contingency(),
    lda_ȳ, y[test]
) |> r3

# ╔═╡ 7b02f449-a686-48df-973a-4ad5442483f2
mutualinfo(
    MITsallisFuruichi(q=1.5), Contingency(),
    lda_ȳ, y[test]
) |> r3
...
manifest_format = "2.0"
project_hash = "b20f0b63e431cfc3aae140f4c5344d106f92c84c"
