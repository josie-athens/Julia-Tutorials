...
# ╔═╡ 68364946-21e2-4211-8d2a-5a92acafc4f2
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ 5759d641-e88f-438c-912c-cec5448873c6
begin
	using StatsKit, AnovaGLM, Effects, StandardizedPredictors
	using DataFrameMacros, Chain, FreqTables
	using RCall, ScientificTypes
end

# ╔═╡ 63b14f5a-47ee-4f58-a7f0-03fbe68356ce
begin
...
# ╔═╡ 29eaf9d4-16f5-4abc-831c-b8172283ad9e
include("pubh.jl");

# ╔═╡ 8d5c0ce4-3014-11ee-282a-69224310593c
md"""
...
# ╔═╡ bf28648c-1ae0-4bc8-a313-0151c8f54276
md"""
## 📖 Main Menu
...
# ╔═╡ 80b45631-fdeb-4882-9d5e-385dc4bda38c
ST = ScientificTypes; schema = ST.schema;

# ╔═╡ 87e33834-c68e-467f-bcd2-8912c9a6c0d7
# ╠═╡ show_logs = false
R"""
...
# ╔═╡ c1b49a6f-e36c-48e4-b856-7e336de8f49b
md"""
# Univariate Analysis
...
# ╔═╡ 32cb6dab-ea80-48f9-9e4f-6d5627ce8ee7
R"""
kfm = read_rds("data/kfm.rds")
""";

# ╔═╡ 6a95739c-eccb-49cb-b0b1-67bfac82b867
@rget kfm; kfm |> schema

# ╔═╡ f218a04d-a5c4-4922-bd82-3e6854ca3fe3
md"""
!!! warning \"Exercise\"
...
# ╔═╡ 3dda7101-9267-4fa6-bc6f-130214e475b7
R"""
kfm %>%
  select(- no) %>%
  cross_tbl(by = "sex") %>% 
  set_caption("Descriptive statistics of the kfm dataset.") %>%
  theme_pubh(2) %>%
  add_footnote("Median (IQR)") %>%
  print_screen()
"""

# ╔═╡ 6ffebc04-78af-40ae-bc1b-b76eb49ea01a
md"We would like to know more about the distributions of our outcome and our primary predictor."

# ╔═╡ 695eb4b4-b14d-4907-892d-f69f31e4b76d
data(kfm) * 
mapping(
	:dl_milk => "Breast-milk intake",
	color=:sex
) *
visual(QQNorm, qqline=:fitrobust, markersize=5, color=:cadetblue) |>
draw

# ╔═╡ 7d563458-a378-4d8e-b95d-d33410e15b56
data(kfm) * 
mapping(
...
# ╔═╡ fdf563d5-b49d-4046-adaa-9ce18a5fb8f1
md"""
!!! tip \"Question\"
...
# ╔═╡ b7d7ba32-adf2-4ed2-8c37-b2effcb5aa24
md"""
!!! note \"Answer\"
...
# ╔═╡ 1041eddf-4a90-4b8c-9954-44675581e857
md"We can plot the linear relationship between maternal weight and breast-milk intake for all the cases (unadjusted)."

# ╔═╡ b00a7541-6f25-42d3-8273-fa3600e9aa97
let
	layers = visual(Scatter, markersize=8, color=:cadetblue) + linear()
	data(kfm) *
	mapping(
...
# ╔═╡ 56b7dee7-dcb5-477e-ac8d-d088ef754756
md"""
...
# ╔═╡ c71a6915-2802-4ba5-8f29-ee39db39305f
md"""
...
# ╔═╡ eb9875f2-86bd-4503-a3be-3566aacc461b
model_1 = lm(@formula(dl_milk ~ mat_weight), kfm)

# ╔═╡ d8cdcdc2-59cf-4a3c-adf7-87623055afcf
model_1 |> r2

# ╔═╡ fa43c1f0-a93a-4c2e-b3f3-d5fa0837e6fb
md"""
!!! tip \"Question\"
...
# ╔═╡ 656c1ae2-8f4b-4eec-a1bb-3a79f98d5675
md"""
!!! note \"Answer\"
...
# ╔═╡ 4f5c9b96-b1f9-46a6-a7a4-cdf2357393fd
md"""
### Centring
...
# ╔═╡ a3e6a34e-d490-437b-90a6-c7670c57cbdd
model_2 = lm(
...
# ╔═╡ ffaba01c-35d9-4674-bdd3-a9dd28e07c97
md"""
!!! danger \"Interpretation\"
...
# ╔═╡ d2ddfc89-958d-460f-84de-44e302659c30
@chain kfm begin
	select(:dl_milk, :mat_weight)
	estat(["Breast-milk intake (dl/day", "Maternal weight (kg)"])
end

# ╔═╡ 98c6e81d-fe50-4f80-9fe0-c5a9a30f10cb
md" YAY !"

# ╔═╡ 7545ab0a-fbe3-44b7-94e5-9e71e3633e2f
md"""
...
# ╔═╡ af3cf652-d883-47dd-8042-08048c96be68
kfm_cont = Dict(
	:mat_weight => Center(),
	:mat_height => Center(),
	:sex => EffectsCoding()
...
# ╔═╡ fdeb7894-5d64-4983-9ad1-d18a6bb9df4d
model_3 = lm(
	@formula(dl_milk ~ mat_weight + sex  + ml_suppl + mat_height),
	kfm; contrasts = kfm_cont
)

# ╔═╡ fa2f3a2d-8915-46a4-a355-6d1a35a09dc6
anova(model_3)

# ╔═╡ a7a97b3b-ccf7-435c-9906-a6707d1f0122
md"""
...
# ╔═╡ 63c2a948-3761-48b3-9279-05a050dc1c93
model_4 = lm(
	@formula(dl_milk ~ mat_weight + sex + mat_height),
	kfm; contrasts = kfm_cont
); anova(model_4; type=3)

# ╔═╡ 9d903831-0501-4df3-93a9-cb5d47ef0dd5
md"We can compare our new model (`model_4`) against our previous one, with an ANOVA test, the $p$-value, is the same as the one from the ANOVA table."

# ╔═╡ 849a5960-4da2-47bc-a0b1-5c660e15a36a
anova(model_3, model_4; check=false)

# ╔═╡ 85d8afc2-733c-4bf4-a0f4-ddd86d8d4459
md"""
!!! danger \"Interpretation\"

	There is no statistical difference between a model that includes milk-supplement and a model that does not contain it ($p$ = 0.269, $F$-test).
"""

# ╔═╡ ef19e877-39ed-477f-8e7b-e5826c71ded3
md"""
We use information criteria (IC) for model simplification. Both Akaike's information criterion (AIC) and Bayesian information criterion (BIC) penalise the number of parameters in the model. The more parameters included in the model, the more penalty is given. It can be calculated with the following formula:
...
# ╔═╡ b292d8ce-1387-4c24-b7a2-7a2b8576ca75
...
# ╔═╡ 7109414d-68fb-4991-b41e-cd16f2e3c214
...
# ╔═╡ ec39b24a-5325-4ca5-a2c9-20f778646878
let
...
!!! danger \"Interpretation\"
	...
	When we have non-parallel fitting curves as those shown in the previous plot, it means that the effect is not the same at different levels of the categorical variable. In other words, sex is an **effect modifier**, as has an **interacting** effect in the relationship between maternal weight and daily breast-milk intake.

Let's add an interaction to our last model.
"""

# ╔═╡ 5d6a4fca-6e77-4253-b7ef-ac59ea9b430c
model_5 = lm(
	@formula(dl_milk ~ mat_weight * sex + mat_height),
	kfm; contrasts = kfm_cont
)

# ╔═╡ 7afd77dc-7304-4b63-bd67-80636aeaae95
md"Let's compare the model with the interaction term against the model without the interaction:"

# ╔═╡ 30cfe5eb-cb9f-4a9d-8fee-71203ab0b066
anova(model_4, model_5; check=false)

# ╔═╡ 27642d51-f259-4078-93da-c04d158d61f9
bic(model_4), bic(model_5)

# ╔═╡ 3174d133-4fbb-4c1f-b4d8-07bc731874e2
md"""
!!! danger \"Interpretation\"
...
# ╔═╡ 95dc607c-0d84-45d9-8274-29ba3bd53657
md"""
## Stratification
...
# ╔═╡ 8847b713-4f82-40db-879e-15b0bdf98f27
md"First we create our subsets:"

# ╔═╡ d3637152-0ec5-4dca-9363-673c236d3e35
boys = @subset(kfm, :sex == "Boy");

# ╔═╡ ff7708a5-2133-41dc-a600-0179478eb5cb
girls = @subset(kfm, :sex == "Girl");

# ╔═╡ 3d00f007-1b6c-4d44-99ed-96ec444f5f16
md"Contrasts for the models (centring):"

# ╔═╡ 0568ec1f-b39b-4e79-b220-02e079f5c188
sex_cont = Dict(
	:mat_weight => Center(),
	:mat_height => Center()
);

# ╔═╡ cc2ec07d-2c66-4c0a-b9b0-7a69478caf8f
model_boy = lm(
	@formula(dl_milk ~ mat_weight + mat_height),
	boys; contrasts=sex_cont
)

# ╔═╡ 34455e63-9fab-4db5-96dc-79b860797852
model_girl = lm(
	@formula(dl_milk ~ mat_weight + mat_height),
	girls; contrasts=sex_cont
)

# ╔═╡ 279816bb-621c-4b51-95b6-ca7d1483051b
md"""
!!! danger \"Interpretation\"

	For boys, maternal weight has a significant positive effect on their daily breast-milk intake ($p$ = 0.018), whereas for girls, maternal weight has no significant effect on their daily breast-milk intake ($p$ = 0.949).
"""

# ╔═╡ 74921e35-aef2-41ac-a3a1-7a17653738b8
md"""
## Diagnostics
"""

# ╔═╡ 1df20a19-2bc7-41a8-87f9-74686834967b
md"The performance data frames:"

# ╔═╡ 1fc5c452-6c47-49cc-aea6-952644f0b740
boy_perf = model_perf(model_boy);

# ╔═╡ 57e9b18a-8ea2-4f5a-a206-19119006d22b
girl_perf = model_perf(model_girl);

# ╔═╡ b3b84991-c8e4-4da0-84ff-adab5b1302fe
md"### Normality"

# ╔═╡ 411065d1-54b7-4b27-ab4e-6e4adf843634
let
	fig = Figure()
...
# ╔═╡ 8595bfeb-637c-4991-af81-9f9483b6e962
md"### Homoscedasticity"

# ╔═╡ ac64d5e7-5744-4930-a31f-f44dbecbee61
let
	fig = Figure()
	...
	AoG.draw!(fig[1, 1], p1, axis=(;title="Boys"))
	AoG.draw!(fig[2, 1], p2, axis=(;title="Girls"))
	fig
end

# ╔═╡ d7e399de-72c3-4efd-a772-ff5e40c3a20b
let
	fig = Figure()
...
# ╔═╡ 48390d80-428c-432a-8cd0-791d1fa1da89
md"### Influential observations"

# ╔═╡ 5d506060-bdce-4029-938a-abfe411cc5d9
let
	fig = Figure()
	...
	μ1 = mean(boy_perf.cook)
	μ2 = mean(girl_perf.cook)

	stem!(ax1, 1:nrow(boys), boy_perf.cook; markersize=8)
	hlines!(ax1, μ1, linestyle=:dash, color=:plum)

	stem!(ax2, 1:nrow(girls), girl_perf.cook; markersize=8)
	hlines!(ax2, μ2, linestyle=:dash, color=:plum)
	
	fig
end

# ╔═╡ 16991891-55a4-4bea-a249-e9cad7440ed0
let
	fig = Figure()
...
# ╔═╡ 4463f1ad-8bfc-4b68-9d80-37602fd83c75
let
	fig = Figure()
...
# ╔═╡ 3fdea0ad-8506-4cc1-8a95-e5a5cd37a367
md"### Performance statistics"

# ╔═╡ 7c4dc596-e68c-47f6-a963-b6f0ed15bd14
@show r2(model_boy); @show r2(model_girl);

# ╔═╡ f56f88e0-0743-4d0b-9c08-3fc4a9c6602e
@show adjr2(model_boy); @show adjr2(model_girl);

# ╔═╡ f1917d48-c479-441f-94f3-9b02dc4f0a91
@show mape(boy_perf); @show mape(girl_perf);

# ╔═╡ 37f2b0ce-167e-4226-8938-f154427d8a4f
@show rmse(boy_perf); @show rmse(girl_perf);
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "369e0fde07730a3a328833922fa446e3f59df60d"
...
git-tree-sha1 = "f8fbdfb14c4a9d62ab22e23954e5f8d5ec2e27d9"
