...
# ╔═╡ fd351141-f03a-4395-b26c-1419400bbe5e
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ 141a8961-8258-4794-9bcc-b39f8c44643d
begin
	using StatsKit, DataFrameMacros
	using Chain, FreqTables
	using RCall
	using  ScientificTypes
	ST = ScientificTypes; schema = ST.schema
	using CausalityTools
end

# ╔═╡ a3c4068d-df03-43f2-9ee9-19bf0bede7b1
begin
...
# ╔═╡ 7792d7c8-2f27-11ee-377c-9dd54ec20402
md"""
...
# ╔═╡ 8a1f6460-f5e8-4ae7-905a-08c63762a18c
md"""
## 📖 Main Menu
...
# ╔═╡ 3d78d7d4-8f6d-43c9-abcd-2cd233c5436b
# ╠═╡ show_logs = false
R"""
...
# ╔═╡ 9cfdecf4-2296-440f-958c-e6f76a1253b3
md"""
...
# ╔═╡ 3656e08b-cc04-4f56-8153-201feb4f6716
md"""
!!! tip \"Example\"

	We will use data on peak knee velocity in walking at flexion and extension.
"""

# ╔═╡ 9f858c51-8d92-435a-8c29-21210e89e9c7
R"""
data(Tuzson)
""";

# ╔═╡ a9c9ce60-46ce-49c7-82b3-e9cb743b2b6a
tuzson = rcopy(R"pubh::Tuzson"); tuzson |> schema

# ╔═╡ f3ea1c74-70d5-4e64-b41b-47ef72cc2547
md"We start by constructing a scatter plot to visualise potential correlations."

# ╔═╡ ee14de2c-da31-4a6f-b816-726cf711b0c6
data(tuzson) *
mapping(
	:extension => "Extension (°/s)",
	:flexion => "Flexion (°/s)"
) *
visual(Scatter, markersize=10, color=:cadetblue) |> draw

# ╔═╡ 2bbfdb35-1820-430f-a199-145df13a3c5d
md"One of the assumptions is that the joint distribution is a normal distribution. We can take a look at the individual distributions as a proxy of the joint distribution."

# ╔═╡ 024d9b57-f5d6-453a-880a-0e74058459fa
let
	f = Figure()
	ax1 = Axis(f[1, 1], xlabel = "Normal Quantiles", 
		ylabel = "Extension (°/s)")
	ax2 = Axis(f[1, 2], xlabel = "Normal Quantiles", 
		ylabel = "Flexion (°/s)")
	qqnorm!(ax1, tuzson.extension, qqline = :fitrobust)
	qqnorm!(ax2, tuzson.flexion, qqline = :fitrobust)

	f
end

# ╔═╡ 6aff8ffc-68d3-49cd-8245-def8ce893d8d
CorrelationTest(tuzson.extension, tuzson.flexion)

# ╔═╡ 8c426e5e-d7da-41b2-816c-c8db4fc3b575
md"""
!!! danger \"Interpretation\"

	There is a correlation of 0.88 (95% CI: 0.71, 0.96, $p$ < 0.001) between the extension and flexion on peak knee velocity in walking.
"""

# ╔═╡ b958f67f-a94b-4e71-8725-e44d0493bed6
md"""
...
# ╔═╡ 041be2ca-ce7d-411f-a5b8-50c8446fccea
md"""
!!! tip \"Example"

	We are going to compare survey questions collected using a six-point response scale; therefore, our variables are ordinal. The dataset `bfi` is part of the `R` package `psych`.
...
# ╔═╡ 111dec68-3ec7-48c1-baf2-5623329da9b2
R"""
...
# ╔═╡ a7b43d38-d4be-466c-820b-cbabec8462ac
bfi = @rget bfi;

# ╔═╡ b8c1ee50-206a-46e9-be4c-d6d051588830
dropmissing!(bfi); bfi |> schema

# ╔═╡ f0402386-f30f-4c39-be06-5211f3ab3e15
md"We are going to compare question A3: Know how to comfort others with question A5: Make people at ease."

# ╔═╡ 14f28cb8-df5d-476b-8cfb-2510bd5c31b6
md"The distribution of **Know how to comfort others** by sex."

# ╔═╡ fcf9349b-f585-4079-ad99-8885a0127f1e
freqtable(bfi, :a3, :sex)

# ╔═╡ 4af7464f-6d4c-4ace-a840-bf9f95370b02
md"The distribution of **Make people feel at ease** by sex."

# ╔═╡ 68198824-6759-4d4e-b720-2503fafaebbf
freqtable(bfi, :a5, :sex)

# ╔═╡ 81d707d2-73d2-4e6b-b815-22d8723ac391
md"To compare the distributions in a single plot, we combine the frequency tables in a new data frame."

# ╔═╡ ee2410d9-844e-4aac-9bd0-00b21ed520ce
bfi_melt = stack(bfi, [:a3, :a5], :sex); first(bfi_melt, 5)

# ╔═╡ 4097419c-fdb9-4cc0-ad33-28ebd0f02742
let
	plt = data(bfi_melt) *
	mapping(
		:value => "",
		color=:variable => "Question", 
		layout=:sex, dodge=:variable
	) *
	frequency()
	draw(plt, axis=(; xticklabelrotation=pi/6))
end

# ╔═╡ 26388f68-af4a-4eb3-a456-9f2da8fa5c27
corspearman(bfi.A3, bfi.A5)

# ╔═╡ 9fed81e9-aee4-470f-82e9-97bbb9a5eb87
md"""
!!! danger \"Interpretation\"

	There is a significant correlation of 0.53 (Spearman's $\rho$ = 0.53) between question A3: *Know how to comfort others* and question A5: *Make people at ease*. Shannon's mutual information between the variables was 0.277 and significant ($p$ < 0.001).
"""

# ╔═╡ 156b171e-6d7a-4e0c-b1f7-455b54616b94
md"""
...
# ╔═╡ ee45bbf0-a7bd-48f6-b5a9-ee4c06438de9
md"""
!!! tip \"Example\"

	We are going to use the `attitude` dataset about answers to questionnaires sent to assess attitudes in a financial organisation.
"""

# ╔═╡ 12298df5-084d-4da3-af52-59e977955da1
attitude = rcopy(R"datasets::attitude"); attitude |> schema

# ╔═╡ c570e292-b8d4-4679-8c81-3f4faaefc115
att_mat = Matrix(attitude);

# ╔═╡ fd20a328-b883-4236-861d-fd937c5a3d5c
corkendall(att_mat)

# ╔═╡ 8e987864-5a55-45e6-8386-34befddc8f27
md"We can use a *heatmap* to construct a correlation plot."

# ╔═╡ 7395952f-a1ff-4d05-abb4-69f36ad4c020
let
	...
	ax = Axis(
		fig[1, 1],
		yreversed=true,
		xticks=(1:7, names(attitude)),
		yticks=(1:7, names(attitude))
	)
	hm = heatmap!(ax, att_cor, colormap=:Derain)
	Colorbar(fig[1,2], hm)
	points = [Point2f(x,y) for x in 1:7 for y in 1:7]
	colors = reshape(ifelse.(att_cor .< 0.0, :white, :black), length(att_cor))
	labels = reshape(string.(round.(att_cor, digits=2)), length(att_cor))
	attrs = (position=points, align=(:center, :center), color=colors,fontsize=14)
	text!(fig[1,1], labels; attrs...)
	fig
end

# ╔═╡ efac8826-b96f-431d-b4fc-f331a1e7a3f6
md"""
!!! tip \"Question\"

	What are the three largest correlations? 
"""

# ╔═╡ 49ea790b-1d5d-467a-8de0-a88e2eb7743d
md"""
!!! note \"Answer\"

	The three largest correlations (Kendall's $\tau$) are between:

	- Rating and Complaints ($\tau$ = 0.65)
	- Learning and Raises ($\tau$ = 0.51)
	- Complaints and Raises ($\tau$ = 0.49)
"""

# ╔═╡ 795e4384-ea6c-48c7-bceb-c2aae2bda5d3
md"""
# Mutual Information
...
# ╔═╡ 426ccbc3-6393-4228-b38f-150748e52cbe
md"""
...
# ╔═╡ fbfc7a67-066b-47fd-a7e5-7e9cdcf6a768
independence(
	SurrogateTest(PearsonCorrelation()),
	tuzson.extension, tuzson.flexion
)

# ╔═╡ aebe621b-85f6-45a9-9586-28049322a77f
md"That give us the same result that we got before. Now let's calculate Shannon's mutual information (entropy). The higher is the mutual information, that indicates that variables are not independent."

# ╔═╡ fc09546b-082e-420e-ade3-8faa09576c1a
independence(
	SurrogateTest(TEShannon(), KSG1()),
	tuzson.extension, tuzson.flexion
)

# ╔═╡ 06eecf1a-b290-4304-89da-611391b58c62
md"""
!!! note

	`TEShannon` is suitted to look at time series. The KraskovStögbauerGrassberger1 mutual information estimator (you can use KSG1 for short) is the $I^{(1)}\kappa$-th nearest neighbor estimator.
"""

# ╔═╡ 324b0276-afcd-4c1f-9c9d-a1a7e0b304dc
md"""
## Categorical variables
"""

# ╔═╡ 7c0848cc-37ca-4d17-954c-caa3d46888f0
md"""
When our variables are categorical, instead of using the Pearson's correlation for the surrogate test, we can test for Shannon's Mutual Information (entropy) and add the `Contingency` function.

Let's go back to the comparison of question A3: Know how to comfort others with question A5: Make people at ease.
"""

# ╔═╡ a8b43e11-c51a-4c47-bd15-c592efc15100
independence(
	SurrogateTest(MIShannon(), Contingency()),
	bfi.a3, bfi.a5
)

# ╔═╡ a9ad68a9-0a32-4609-8d18-39e1da61fcf1
md"""
...
# ╔═╡ 2dd434ff-4386-4bfa-9e24-38768d54d629
independence(
	SurrogateTest(MIShannon(), Contingency()),
	att_mat[:,1], att_mat[:,2]
)

# ╔═╡ 9da52d83-149b-4b7c-8807-de97ffb1ffa9
names(attitude)

# ╔═╡ c4b48341-0e37-4060-aeb5-259cf2470e1d
md"""
# Autocorrelation
...
# ╔═╡ a5a980b1-4e48-47a5-91d7-2274d9787b53
md"""
!!! tip \"Example\"

	We are going to use the dataset `CVD` from the `season` `R` package, which has records of the monthly number of deaths from cardiovascular disease (CVD) in people aged 75 years and over in Los Angeles.
"""

# ╔═╡ 8b15c2f1-d272-494a-bd1d-3c4ce03eb7db
cvd = rcopy(R"season::CVD"); cvd |> schema

# ╔═╡ 063857f9-4bcd-4a8c-bc9e-8bd494ed5b1b
md"First, we can look at the time series."

# ╔═╡ d218b534-dc16-4a5e-b2eb-895000f0df3b
data(cvd) *
mapping(
	:yrmon => "Date",
	:cvd => "CVD Deaths"
) *
visual(Lines, color=:cadetblue) |>
draw

# ╔═╡ 5eed4bec-1d07-4efb-8f50-1f3710097b3e
md"Is better to use the autocorrelation function (ACF) on a *detrended* series. Our time series does not show any trend, but we still are going to remove the mean (*centring*). In `autocor`, the default is: `dmean=true`."

# ╔═╡ 4847bf0b-b522-4a0c-95b1-43e386e22b87
let
	fig = Figure()
	ax = Axis(
		fig[1,1],
	...
	stem!(ax, autocor(cvd.cvd, [1:1:24;]))
	hlines!(0.2, color=:plum, linestyle=:dash)
	hlines!(-0.2, color=:plum, linestyle=:dash)
	fig
end

# ╔═╡ f5217ad8-b783-45ce-8277-d76a8fca1b7e
md"""
!!! danger \"Interpretation\"

	The time series has a season component. The cycles are observed every 12 months (when the correlations are higher).
"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "5485c34a1bb572ba04bf672725fe83feab826cf8"
...
[[deps.CausalityTools]]
deps = ["Accessors", "Combinatorics", "ComplexityMeasures", "DSP", "DelayEmbeddings", "Distances", "Distributions", "DynamicalSystemsBase", "Graphs", "HypothesisTests", "LabelledArrays", "LinearAlgebra", "NearestNeighbors", "Neighborhood", "Random", "RecurrenceAnalysis", "Reexport", "Scratch", "SimpleDiffEq", "SpecialFunctions", "StateSpaceSets", "StaticArrays", "Statistics", "StatsBase", "Test", "TimeseriesSurrogates", "Wavelets"]
git-tree-sha1 = "55eb96d092953d35e4ec169b7edb5251f8b0a852"
...
deps = ["DelayEmbeddings", "ForwardDiff", "LinearAlgebra", "Roots", "SciMLBase", "SimpleDiffEq", "SparseArrays", "StaticArrays", "Statistics"]
git-tree-sha1 = "698c3ec275cf0e0784bdfdaf5fb30ec551fa5142"
...
git-tree-sha1 = "a01a69eda3937b3f022c6fdc7b1d4946385107d0"
...
git-tree-sha1 = "b007c79f1d7ae55f4fb02e6d16bdf9456c5d0b54"
