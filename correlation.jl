...
# ╔═╡ f4c52b99-25bb-4636-9bfe-ba1d375af8a9
...
# ╔═╡ ac814354-2c8e-4b91-aebf-7eaf125f7c57
# ╠═╡ show_logs = false
R"""
...
# ╔═╡ aa8b2ee7-3deb-42bc-9f5e-36faa9022004
md"""
...
# ╔═╡ c7a0daa8-10de-4f28-a0a9-acb22ba78feb
R"""
data(Tuzson)
""";

# ╔═╡ a008a05c-9bb5-458e-8f42-3f0422ccc667
tuzson = rcopy(R"pubh::Tuzson"); tuzson |> schema

# ╔═╡ efc4f39e-c9ba-47b2-92ed-2b4d32c7493b
md"""
We start by constructing a scatter plot to visualise potential correlations.
"""

# ╔═╡ 3395fb4d-b2c3-48e0-b377-9cd59fe7bec9
@df tuzson scatter(
	:extension, :flexion,
	xlabel="Extension (°/s)",
	ylabel="Flexion (°/s)",
	mc=:midnightblue, ms=3, leg=false
)

# ╔═╡ 13042129-4ef5-4cb8-8784-fd8de46b188d
md"""
...
# ╔═╡ 00bef115-d0ec-471e-af5b-080734186997
let
	p1 = qq_plot(tuzson.extension, ylab = "Extension (°/s)")
	p2 = qq_plot(tuzson.flexion, ylab = "Flexion (°/s)")

	plot(p1, p2, layout=2)
end

# ╔═╡ c34188cd-bb4c-4b59-b039-d51b0f0808e9
CorrelationTest(tuzson.extension, tuzson.flexion)

# ╔═╡ 23c3fa1c-c021-4108-81e5-e03461bc2e22
md"""
...
# ╔═╡ 4240ace7-8122-4c44-accf-83e8d88b6360
md"""
...
# ╔═╡ bc13688b-2150-4b6f-ad71-2549fcf960ba
md"""
!!! tip \"Example\"

	We are going to compare survey questions collected using a six-point response scale; therefore, our variables are ordinal.
...
# ╔═╡ ad5bdd20-e4e3-430b-9daf-026019476a32
R"""
...
# ╔═╡ 42885d10-fd96-4e19-97f6-2f2a3e1f82df
bfi = @rget bfi;

# ╔═╡ 5eae0aa8-5846-4a8b-8f35-b8e8013975cd
dropmissing!(bfi); bfi |> schema

# ╔═╡ adc34b43-64ce-44e6-a70b-bc0058c29a67
md"""
We are going to compare question A3: *Know how to comfort others* with question A5: *Make people at ease*.

The distribution of **Know how to comfort others** by `sex`.
"""

# ╔═╡ 874e6b02-ff15-4e80-b41b-935bed3bde78
tabulate(bfi, :a3, :sex)

# ╔═╡ 5cc07d95-f8da-498b-947f-cab14c64aca9
md"""
The distribution of **Make people feel at ease** by `sex`.
"""

# ╔═╡ 1cc0f844-630d-4699-b413-defb26e94f32
tabulate(bfi, :a5, :sex)

# ╔═╡ 4f7d056f-5f82-41be-8d62-91d4d0c8b4a4
md"""
To compare the distributions in a single plot, we combine the frequency tables in a new data frame.
"""

# ╔═╡ 29d44b8a-c9a1-40b2-a865-2ff957a5b6d3
bfi_melt = stack(bfi, [:a3, :a5], :sex); first(bfi_melt, 5)

# ╔═╡ 1becbcac-f3e3-40cc-a579-de4e8c14fe3c
let
	a3 = @combine(
		groupby(
			@subset(bfi_melt, :variable=="a3"), 
			[:sex, :value]), :count = @nrow
	)
	a5 = @combine(
		groupby(
			@subset(bfi_melt, :variable=="a5"), 
			[:sex, :value]), :count = @nrow
	)
	p1 = @df a3 groupedbar(
		:sex, :count, group=:value,
		ylabel="Frequency", title="A3"
	)
	p2 = @df a5 groupedbar(
		:sex, :count, group=:value,
		ylabel="Frequency", title="A5"
	)
	
	plot(p1, p2, layout=2)
end

# ╔═╡ 02be1b18-883f-420c-a060-19a9095d0eef
corspearman(bfi.A3, bfi.A5) |> r3

# ╔═╡ 0e9a391b-2b1f-4c9d-ae8d-065196a33168
md"""
!!! danger \"Interpretation\"

	There is a significant correlation of 0.53 (Spearman's ρ = 0.53) between question A3: *Know how to comfort others* and question A5: *Make people at ease*. Shannon's mutual information between the variables was 0.277 and significant ( $p$ < 0.001).
"""

# ╔═╡ 0e18489f-6dff-44a8-9f49-a6732e9c9b0a
md"""
## Kendall's Correlation

Kendall's τ is a non-parametric correlation that should be used instead of Spearman's when the sample size is relatively small, and the data has a relatively large number of ties.
"""

# ╔═╡ 299b2d2e-43fb-4d3f-9915-a104e6aae0e7
md"""
!!! tip \"Example\"
...
# ╔═╡ e999ab5a-1fdd-4868-aa30-6612edcf49a4
attitude = rcopy(R"datasets::attitude"); attitude |> schema

# ╔═╡ c42c57a7-8e25-44d7-8e1d-6834292559d9
att_mat = Matrix(attitude);

# ╔═╡ 7fef04e2-7384-4df9-9413-a5ea64a4aa59
corkendall(att_mat)

# ╔═╡ b3ab1f9c-1887-4be5-8890-181a04cf7c8d
corrplot(att_mat, grid=false, ms=1.5)

# ╔═╡ e61af040-e98a-4b2a-9394-e6df0d493587
md"""
We can use a *heatmap* to visualise correlations.
"""

# ╔═╡ d6e94a2d-0667-4b40-85d6-b34226e90cc3
heatmap(
	corkendall(att_mat),
...
# ╔═╡ 5de6270f-4a44-4fec-839e-72eec81855aa
md"""
!!! warning \"Question\"
...
# ╔═╡ 562c6457-04d0-4d39-8656-2aee4f5b160e
md"""
!!! hint \"Answer\"

	The three largest correlations (Kendall's $\tau$) are between:

	- Rating and Complaints ($\tau$ = 0.65, 95% CI: 0.49, 0.78, $p$ < 0.001)
	- Learning and Raises ($\tau$ = 0.51, 95% CI: 0.30, 0.67, $p$ = 0.001)
	- Complaints and Raises ($\tau$ = 0.49, 95% CI: 0.27, 0.66, $p$ = 0.001)
"""

# ╔═╡ 309a91b5-655e-481a-a628-b0f6cccf091f
md"""
# Mutual Information
...
# ╔═╡ 70182b75-f46a-4f62-9d91-9ceb2fe10986
independence(
	SurrogateTest(PearsonCorrelation()),
	tuzson.extension, tuzson.flexion
)

# ╔═╡ 80738d5f-7002-4009-9de4-7bc9d1a7626f
md"""
...
# ╔═╡ be360a6b-c91c-49dc-aa6c-bb1594c86506
independence(
	SurrogateTest(TEShannon(), KSG1()),
	tuzson.extension, tuzson.flexion
)

# ╔═╡ 8b241f4f-3c3e-4349-a85d-03a2645c9f0a
md"""
!!! note
...
# ╔═╡ b7b61bd8-a947-4773-8aa7-753e434e1d34
md"""
...
# ╔═╡ 2d097668-c159-4653-8c76-7d9497e1c285
independence(
	SurrogateTest(MIShannon(), Contingency()),
	bfi.a3, bfi.a5
)

# ╔═╡ 94709ba3-4404-407a-a693-6d5a5bc552fc
md"""
...
# ╔═╡ 0b229ac5-2b74-4ca4-8584-b6f37d1d1053
names(attitude)

# ╔═╡ e243183f-8fc4-4865-8429-e694ef092aee
independence(
	SurrogateTest(MIShannon(), Contingency()),
	att_mat[:,1], att_mat[:,2]
)

# ╔═╡ ad1fd8a2-7777-4215-9255-a6622941771c
md"""
# Autocorrelation
...
# ╔═╡ ad49f817-a8a0-48e5-bf95-34314152eaa1
md"""
!!! tip \"Example\"
...
# ╔═╡ e6b0dc49-7edb-4254-8cae-9d92272ef616
cvd = rcopy(R"season::CVD"); cvd |> schema

# ╔═╡ c9e6beb4-5b0f-4d49-80f4-9a120846a396
md"""
First, we can look at the time series.
"""

# ╔═╡ 76a44e6b-28c1-47e8-8c18-9e0d212b0add
@df cvd plot(
	:yrmon, :cvd,
	xlabel="Date",
	ylabel="CVD Deaths",
	marker=2, mc=:firebrick, 
	lc=:indianred, lw=2, leg=false
)

# ╔═╡ 69bedb41-1a26-4408-b519-4f3a25c28027
md"""
Is better to use the autocorrelation function (ACF) on a *detrended* series. Our time series does not show any trend, but we still are going to remove the mean (*centre*). In `autocor`, the default is: `dmean=true`.
"""

# ╔═╡ b55af31c-e8a8-4894-813f-acb2f07288b3
let
	plot(
		[1:1:24;],
		autocor(cvd.cvd, [1:1:24;]),
		line=:stem, marker=2, leg=false,
		mc=:firebrick, lc=:indianred,
		xlabel="Lag", ylabel="ACF"
	)
	
	hline!([0], lc=:black)
	hline!([-.2, .2], linestyle=:dash, lc=:cadetblue)
end

# ╔═╡ 68d67740-936b-405d-baa9-13db3b4d178d
md"""
!!! danger \"Interpretation\"
...
manifest_format = "2.0"
project_hash = "1d066505a624a593b14f976211121164eeeb6b51"
...
git-tree-sha1 = "9ad169e6d4d8a91fc14b0c4d926009970aa7d204"
