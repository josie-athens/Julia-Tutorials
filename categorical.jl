...
# ╔═╡ 39046b70-22cd-426b-bc89-167ddaafbfb7
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ c6b40625-1d14-4437-bb10-9cba6952e055
begin
	using StatsBase, DataFrames, DataFrameMacros, MLJ
	using RCall, TexTables, CategoricalArrays, GLM
	using AnovaGLM, Effects, MultipleTesting, HypothesisTests
	using StatsPlots
	using MLJ: schema
end
...
# ╔═╡ 149b7f4a-2e59-11ee-3c2c-5fc77d22513f
md"""
# Binary Outcomes
...
# ╔═╡ bba39fd0-bd35-43f0-834d-eead693e87e2
md"As `race` has three categories (levels), we will generate a new race variable (which we will call `Race`) that collapses two of the categories (white and non-white)."

# ╔═╡ f5684644-ae56-4bcf-93fd-80a89da253bf
tabulate(bernard, :race)

# ╔═╡ 80303893-e472-425c-8edc-0e7444ef198d
bernard.Race = recode(
  	bernard.race, "African American" => "Non-White", "Other" => "Non-White"
);

# ╔═╡ 021efa43-2831-4dd4-b93e-f78e0ba1ea75
tabulate(bernard, :Race)

# ╔═╡ 204405ac-e02e-4891-b32e-41ad3bd6601a
r3.(BinomialTest(162, nrow(bernard)) |> confint)

# ╔═╡ 2df32411-22b3-41a8-82ed-7d8356200688
md"""
## Hypothesis Tests
...
# ╔═╡ a03c706b-3aac-43e8-9849-af486095b274
BinomialTest(162, nrow(bernard), 0.4)

# ╔═╡ 242343be-8e95-49da-af9b-2fef0c4bec1d
md"""
# Epidemiology
...
# ╔═╡ 5d2a65bb-7a64-4a0e-8925-9c8113a075b5
md"""
!!! tip \"Example\"
...
# ╔═╡ b1c39f8a-4bac-46b9-a73c-32e12c889012
cancer = readr.read_rds("data/cancer.rds") |> rcopy; cancer |> schema

# ╔═╡ 6cf93e2d-e257-4ac4-895f-9a9a3ea66f2a
md"Let's take a look at the frequency table of `alcohol`:"

# ╔═╡ 8ebc3460-db82-43e2-9794-1256fd5f9578
tabulate(cancer, :alcohol)

# ╔═╡ f23562a6-1693-44b2-acbf-6eb64eb13ef2
md"""
!!! warning \"Exercise\"
...
# ╔═╡ 8fb70b62-a105-4268-8e1b-4560f3e7fdd0
md"""
```julia
cancer.etoh = recode(
  	cancer.alcohol, "40-79" => "40+", 
	"80-119" => "40+", "120+" => "40+"
);
```
""" |> hint
...
md"""
It is possible to make calculations, using the information from a standard two-by-two table (i.e., as an immediate command).
...
# ╔═╡ 6463b02e-b34d-48ed-a46f-a768e035d0b6
epitools.epitab(
	epitools.epitable(cancer.etoh, cancer.status, rev="both"),
	method="oddsratio", verbose = true
)

# ╔═╡ 9346fb7a-37cd-4313-bdf4-9aec26c948ad
odds_ratio(171, 389, 29, 386) |> r3
...
# ╔═╡ 5bd1f453-db3f-48ba-8d13-d23616301307
md"""
...
# ╔═╡ b6cc5494-6a9b-4e6c-ab29-c28cd0f4fdc1
md"""
...
# ╔═╡ c61aa2b9-9ed5-420f-a473-31acd45dd52c
pubh.mhor(@formula(status ~ age/etoh), data=cancer) |> rcopy

# ╔═╡ 7c92b165-9f86-417a-8ef6-87e6bb0671f0
md"The *formula* used in `mhor` uses the syntax: `outcome ~ strata/exposure`, both the outcome and the exposure have to be binary (factors with two levels). The confounding variable (`strata`) has to be categorical (a factor) and can have more than two levels."

# ╔═╡ 1180e263-8cdc-4de3-b286-13e0601b4c97
md"""
!!! note
...
# ╔═╡ f1d1c030-091f-4f56-ad75-862f56f8c1cd
md"""
...
# ╔═╡ decf92a3-423e-4f76-ad89-67ea207dc205
md"""
!!! tip \"Question\"
...
# ╔═╡ 2060f031-99f3-4311-beaf-9a9bb3f8cfdf
md"""
Yes! The raw OR was 5.85 when we control for age the actual effect of alcohol is an OR = 6.70 (95% CI: 4.28, 10.51).
""" |> hint

# ╔═╡ d7ae8652-ffb5-4d4a-8de7-1307b6fb9201
md"Let's take have a deeper look at the number of cases per exposure group (`etoh`), stratified by age and then, in particular, in the first age group."

# ╔═╡ 99e7e721-07cb-4e1b-98db-e1d08bdb0ed4
tabulate(@subset(cancer, :age == "25-34"), :etoh, :status)

# ╔═╡ dd6711ac-2f1d-4e3a-8026-bbcc94adb4f6
md"""
!!! tip \"Question\"
...
# ╔═╡ 9d08b0f4-8161-4ad8-930c-9d00ed32686a
md"""
...
# ╔═╡ ba5384c1-f2e6-4400-ba61-f14e71d05e88
md"""
...
# ╔═╡ 4fadfa80-4764-403e-954a-b1b6285aefc4
levels(cancer.age)

# ╔═╡ e79f5de5-b9f3-4744-a6cb-ad79145616e4
cancer.Age = recode(cancer.age, "25-34" => "25-44", "35-44" => "25-44");

# ╔═╡ 088e04f7-7469-49bf-a393-f3ba21935f5e
levels!(cancer.Age, ["25-44", "45-54", "55-64", "65-74", "75+"]);

# ╔═╡ 863f9104-6331-4de2-8ccd-91758790be15
pubh.mhor(@formula(status ~ Age/etoh), data = cancer) |> rcopy

# ╔═╡ dd462df6-a56d-4bb8-9d77-5891acd32523
md"""
## Cohort Studies

!!! tip \"Example\" 

	We can look at the effect of the smoking status of mothers on low birth weights of the babies.
"""

# ╔═╡ 8ee84640-c756-46c9-b5b0-2288ce9cddd2
birthwt = readr.read_rds("data/birthwt.rds") |> rcopy; birthwt |> schema

# ╔═╡ e7da0f85-6ad7-45a1-ab1d-dcf439f68e42
md"Let's take a look at the contingency table for the exposure (`smoke`) and the outcome (`low`)."

# ╔═╡ d58ff8fb-cfc1-4552-b6b1-daa6675ebf1a
tabulate(birthwt, :smoke, :low)

# ╔═╡ 75ac6b73-0f48-4068-a6f7-cb950d58fc43
md"For a cohort study, we use relative risk (risk ratio) as the measure of association."

# ╔═╡ 4f250835-905f-40da-b1ad-48d4f4086560
epitools.epitab(
	epitools.epitable(birthwt.smoke, birthwt.low, rev="both"),
	method="riskratio", verbose = true
)

# ╔═╡ c2bca6eb-efec-41c3-92ca-60ae786d1c6f
relative_risk(30, 44, 29, 86) |> r3
...
# ╔═╡ a194f4a3-6c0e-4777-8877-dfef671962d0
md"""
Yes, it is. Babies born from mothers who were smokers were significantly more likely to have a low birth weight than babies born from non-smoker mothers ($p$ = 0.040, $\chi^2$-test). The relative risk of having low birth weight between babies from smoking mothers over non-smocking ones was 1.61 (95% CI: 1.06, 2.44).
""" |> hint

# ╔═╡ c8220428-4441-4b2f-92ac-3371245a6544
md"## Tests for Association and Linear Trend"

# ╔═╡ d46f0eed-58d4-458e-8c1c-22f9168c03e5
md"""
!!! tip \"Example\"
...
# ╔═╡ aa16985d-b3e2-4066-88a8-0651db762a5c
begin
...
# ╔═╡ 41a95534-7aa1-4ae8-b517-eb07af23cea4
coerce!(breast, :Weight => Multiclass);

# ╔═╡ b7550e27-5f9a-40ac-bbc2-e36dddb91dd4
levels!(breast.Weight, ["Underweight", "Normal", "Over (11-24%)",
		"Over (25-39%)", "Over (> 39%)"]);

# ╔═╡ 1b1c4ad4-c9b5-46ab-a218-c6cbfc237bf1
md"The first column, correspond to the number of observations on each combination of categories (weight). We can `expand` the dataframe in `R`:"

# ╔═╡ 489cc00f-a0af-4b28-8f3d-36318e8d6462
breast2 = pubh.expand_df(breast) |> rcopy;

# ╔═╡ 98547da1-03c7-4075-b9d7-e386d5d176ea
md"The corresponding contingency table is:"

# ╔═╡ 8d5c73dc-3d8a-4197-887b-f02b822c77fb
levels!(breast2.Weight, ["Underweight", "Normal", "Over (11-24%)",
		"Over (25-39%)", "Over (> 39%)"]);

# ╔═╡ f183bd93-f660-4490-af7c-2c65d1ca2274
tabulate(breast2, :Weight, :Biopsy)

# ╔═╡ d761bfe8-68c3-437d-9cae-82467a59d7ec
md"With `odds_trend` we can calculate the odds ratio per each group against the reference (underweight)."

# ╔═╡ e1dccca9-b52e-434b-bceb-f95e913c0ac0
R"""
breast_df = odds_trend(Biopsy ~ Weight, data = $breast2, method = "wald")$df
""";

# ╔═╡ 9597f543-4ba2-4323-a5c3-ad48eccb96e4
@rget breast_df

# ╔═╡ b5170b1d-3821-4aae-8976-fe0ed697e4b6
@df breast_df plot(
	:Exposure, :OR, marker=3,
	mc=jama[1], lc=jama[1],
	leg=false, xrot=45,
	xlabel="exposure",
	ylabel="OR"
...
# ╔═╡ d4195226-d21e-41d7-955c-f5072bbe188a
md"""
!!! danger \"Interpretation\"
...
# ╔═╡ 60d658f7-a313-4f95-91c3-ebdcbb61c44c
md"""
!!! note
...
# ╔═╡ 88d8b293-2bb9-4eb4-8dfa-7375dcdf05de
coerce!(
	breast2,
	:Biopsy => Multiclass,
	:Weight => Multiclass
...
manifest_format = "2.0"
project_hash = "5aeb2dc44b8bec9eb27957bae76b1ca70a651209"
...
git-tree-sha1 = "60e797e3c5ec8844b6f0a70f8953e1d666fa0648"
