...
# ╔═╡ ec1142e4-beef-11ee-1272-2d3f25f2ab5b
md"""
# ODE's : Examples of Biological Models 
...
# ╔═╡ 2d079bf5-2c64-4832-8134-57474fd7d5da
md"""
!!! note

	- ẋ represents the instant change in the population of $x$, the **prey** over time.
	- ẏ represents the instant change in the poupulation of $y$, the **predator** over time.
	- α is the rate of growth of the prey.
	- β is the rate at which predators kill prey.
	- γ is the rate of growth of predators due to killing prey.
	- δ is the rate of death of predators.
"""

# ╔═╡ c84060ad-d806-4d01-9249-e98dbc91fb18
...
# ╔═╡ 9feecfa2-fe39-43d3-a06f-d4d316d1e426
md"""
## Parameters
...
# ╔═╡ 136fd95c-9ade-4af3-afcd-86112c4f636b
lotka_p = (α = 0.1, β = 0.005/60, γ = 4e-5, δ = 0.04);

# ╔═╡ 95d82a06-b4df-463b-95e1-08a6ed33b481
lotka_u0 = [200, 600];

# ╔═╡ edd04fcd-5bd8-40e5-8688-695021d270a5
lotka_tspan = (0.0, 360.0);

# ╔═╡ 9f60d6e3-8ded-4cbb-8c10-c7580d7154f4
md"""
## Simulation
"""

# ╔═╡ 73e50079-4d89-4203-85b0-dca85ae08d50
lotka_prob = ODEProblem(lotka!, lotka_u0, lotka_tspan, lotka_p);

# ╔═╡ ca09137c-996c-4ecd-9282-ebfb5f19a2ee
lotka_sol = solve(lotka_prob, reltol=1e-8, abstol=1e-8);

# ╔═╡ 216411f1-b8d2-4534-88cc-6ca793a46beb
md"""
We find the vector with times in `t`:
"""

# ╔═╡ f618df1d-a72a-468e-9f89-e63100932ce6
lotka_sol.t[1:5]

# ╔═╡ 2b7a8f87-5860-4c52-88be-71922e5ce8be
md"""
The matrix for the solutions is in `u`. The first column correspond to the prey and the second column to the predators.
"""

# ╔═╡ 2bf65cd6-9962-4fb1-959a-9a5d49e3138b
lotka_sol.u[1:5, :]

# ╔═╡ 0126c635-8ab6-4e96-b71f-d79c7fa79ff4
md"""
## Visualisation
"""

# ╔═╡ 71d93f5b-b909-4168-96a4-94177056a779
md"""
!!! note

	It is possible to construct the plot directly from the ODE solution. Defining the data frame, avoids confussion, though.
"""
...
md"""
We can also construct the corresponding data frame first.
"""

# ╔═╡ 2ed2beb8-e414-421e-9c4f-2c925b23353f
begin
...
# ╔═╡ 648bd850-f1e3-4d07-b08e-0d892248e8ad
lotka_melt = stack(lotka_df, Not(:timestamp)); lotka_melt |> head

# ╔═╡ 81ae7767-1ac9-4865-b47d-ed1b3debbd77
@df lotka_melt plot(
...
# ╔═╡ 601f3a3d-2eef-4a4d-b91b-aff9ee35e246
md"""
```math
\begin{gather}
\frac{dS}{dt} = -\beta IS \\
...
# ╔═╡ f86b0a72-00ed-4493-a12c-1d87f958a5b9
...
# ╔═╡ 5539d8f5-cd1e-441a-831c-b005517dc1db
md"""
## Parameters
"""

# ╔═╡ c5900047-7b0e-4746-977a-7c8014b49f82
begin
	sir_p = (β = 0.01, γ = 1/8)
	sir_u0 = [200, 1, 0]
	sir_tspan = (0.0, 90.0)
...
# ╔═╡ 7617d60b-f87a-44e9-ab66-4f41e01c3a55
md"""
## Simulation
"""

# ╔═╡ 2f235671-55e4-406a-9aa3-dfbba1d5311b
begin
	sir_prob = ODEProblem(sir!, sir_u0, sir_tspan, sir_p)
	sir_sol = solve(sir_prob)
end;

# ╔═╡ b38d6efa-ea4f-4323-bf17-d57d244a2a79
md"""
## Visualisation
"""

# ╔═╡ 1c3f2f2e-8eb4-45dd-9fec-4e676cb0412b
plot(
...
# ╔═╡ 1e11dc5a-6f8a-4876-8009-e22670f932b5
function sl!(δ, γ, t)
  	exp(-δ*t) + (1/γ) * exp(-0.001*t)
end;

# ╔═╡ 7a52ec97-f89f-4d20-a9c5-aa6ec3f58187
...
# ╔═╡ c2a7e648-0b18-435d-ae8c-a17883ad2a40
md"""
## Parameters
"""

# ╔═╡ 62480bda-4f23-4ed1-9799-dc90f4141def
begin
...
# ╔═╡ d49fc571-6b50-4335-b56b-7c70f948b509
md"""
## Simulation
"""

# ╔═╡ 1e49a5ed-b3c2-49e0-a3be-21d4288c159a
begin
	hiv_prob = ODEProblem(hiv!, hiv_u0, hiv_tspan, hiv_p)
	hiv_sol = solve(hiv_prob, reltol=1e-8, abstol=1e-8)
end;

# ╔═╡ 3eba4c8e-d289-4a18-a586-ef1021402be9
md"""
## Visualisation
"""

# ╔═╡ dab7c5e5-06f2-46d0-aadf-60343d2ba43d
plot(
	hiv_sol, yaxis=:log10,
...
# ╔═╡ 34c7dbf1-74e9-4c2f-852f-f1ec9f6f04cf
@df hiv_df plot(
	:time, :Vb,
	xlab="Time (days)",
	ylab="Virus in Blood",
