...
# ╔═╡ 797dccc9-90f9-494a-85bf-9593550f1930
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ 27fb29c4-385f-4ef6-a717-b5873b0641d1
begin
	using StatsBase, Tidier, CSV
	using DataFrames, TexTables, RCall, MLJ
end

# ╔═╡ 82f351c3-fdff-47eb-8ac0-44e0e960d77a
begin
...
# ╔═╡ aeef9ece-45b8-43d1-b05f-cdef433c7039
using PairPlots

# ╔═╡ 0f81fa1a-0e2f-11ee-3c70-1d756ce6abf4
md"""
# Graphical Analysis
...
# ╔═╡ 29f60f73-c678-4396-9a0e-73959c51a3f0
md"""
## 📖 Main Menu
...
# ╔═╡ 9390b305-b13a-4365-8fb1-78ba24442ea0
begin
	@rimport readr
	@rimport pubh
	import GLM.@formula
...
# ╔═╡ e538e6d6-5889-479c-b5e5-a7156d049626
md"# Data"

# ╔═╡ b867fbb5-b74d-421f-aaf7-56d961333c32
md"""
...
# ╔═╡ 3d4f63f4-a36d-427d-b221-0741ef569fb1
wcgs = readr.read_rds("data/wcgs.rds") |> rcopy; wcgs |> schema

# ╔═╡ d77f45a7-a450-4f23-8e0c-f13c4d3f5c1e
md"""
...
# ╔═╡ b4729f52-faf4-4f51-94fd-d3ec8a624fb0
kfm = readr.read_rds("data/kfm.rds") |> rcopy; kfm |> schema

# ╔═╡ 734cd61c-749e-4cda-8771-57ab1585b964
md"Data on reported cases of influenza by age group in the 1957 pandemic in England and Wales."

# ╔═╡ e26a3a20-6f32-4a8f-a301-b0fb17387dbf
flu = DataFrame(CSV.File("data/fluraw.csv")); flu|> schema

# ╔═╡ 9e0d8214-56de-4cd4-a16c-77dd6d337be6
md"Data on birth weights from newborns in a London Hospital."

# ╔═╡ ee9e0833-e1a7-45fa-91d8-7951e3a3643f
birth = readr.read_rds("data/birthwt.rds") |> rcopy; birth |> schema

# ╔═╡ 7ec5259e-0f2e-4aec-9f57-6acfe53385da
md"Data on the energy expenditure in groups of lean and obese women."

# ╔═╡ 78d9f3ea-f1bf-4e5a-9c1c-7c1fc55db96a
energy = rcopy(R"ISwR::energy"); energy |> schema

# ╔═╡ 7f622b12-daa8-4462-9bfe-9e87fea14615
md"Fisher's iris dataset on measurements for three species of iris."

# ╔═╡ 13a38725-1a29-463e-8056-0a80d229baab
iris = rcopy(R"datasets::iris"); iris |> schema

# ╔═╡ ad389c56-0c96-4600-a3f0-bf03b53b007d
md"Data on air quality and weather conditions in New York, recorded May to September 1973."

# ╔═╡ df5b3d0c-0a9e-4116-b4ce-30767973f86a
begin
...
# ╔═╡ 78d088dc-d9d6-4d36-8a15-a0f9b85907ae
md"""
# Distributions
## Histograms
"""

# ╔═╡ 1be3852c-fdca-4a6a-b2e6-e75da6908337
data(birth) *
...
# ╔═╡ 6108531e-f64b-4c12-b20e-e284a6035575
md"""
!!! note
	...
	- `key_args` are optional arguments to adjust the plot like symbols, lines, colours, etc.
"""

# ╔═╡ d293fd75-e69d-49c0-a714-7ddb67890a95
md"The default for histograms is to show the absolute frequency (counts):"

# ╔═╡ 7812f2e1-d391-4837-bacf-7dda9b426246
data(wcgs) *
...
# ╔═╡ 5cd1cb34-0cba-4ffd-9bb8-50c99ed5a95f
md"We can compare distributions using the density or the probability density function (pdf), as shown here:"

# ╔═╡ 0f0811a7-66f3-4804-b23d-25fe5201f309
data(wcgs) *
mapping(
	:sbp => "SBP (mm Hg)",
...
# ╔═╡ e1bf2c8b-3eb9-4533-bb11-e612cc77dc42
md"We have a better plot when we use *faceting*, i.e., plotting the distributions in two different panels:"

# ╔═╡ 334e051c-18ec-4176-8ae8-66817b882d4b
data(wcgs) *
mapping(
	:sbp => "SBP (mm Hg)", 
	layout=:chd => "CHD"
...
# ╔═╡ 13573db2-5729-4361-83a8-2c666c6e180b
data(wcgs) *
mapping(
	:sbp => "SBP (mm Hg)", 
	layout=:chd => "CHD"
...
visual(color=:cadetblue) |>
draw

# ╔═╡ dded799d-7328-4d59-bf98-06ac4364bd59
md"""
!!! important

	Function `density` is shared with other packages, hence, we explicitly tell `Julia` to use the one from `AlgebraOfGraphics`.
"""

# ╔═╡ 96514305-3757-46a2-8179-90f4bda66e55
md"Diastolic blood pressure (DBP) is skewed to the right (see the QQ-Plot). In the following example, we log-transform DBP."

# ╔═╡ 19012c0e-a30b-4f7f-8f8a-fd68224cb1ef
data(wcgs) *
mapping(
	:dbp => log => "log (DBP)",
	color=:chd => "CHD", dodge=:chd
...
# ╔═╡ b0300c30-c3a1-4c1a-affc-9d327aa21364
md"""
!!! important
...
# ╔═╡ f09e5fef-a0a5-4b38-aed5-2c8e75861b48
data(@filter(wcgs, !ismissing(chol))) *
mapping(
...
# ╔═╡ 8ff5b6fa-6c74-4f6b-a89c-1bb6fbc2e7c7
md"""
## QQ-Plots
...
# ╔═╡ b988e75a-be6e-48a1-8dbd-e58592839ef5
md"""
!!! warning
...
# ╔═╡ c58a6632-a03d-4a40-b2a0-280c5bcd6838
let 
	plt = data(birth) * mapping(:bwt) *
	visual(QQNorm, qqline=:fitrobust, markersize=5, color=:cadetblue)
...
# ╔═╡ e1d28e31-90dc-4fa1-b6d4-245d78ad6986
md"Right-skewed variables, show an upper right curve in the QQ-plot:"

# ╔═╡ 22712b98-2503-4388-8bb6-db04e5c22118
let
	plt = data(wcgs) * mapping(:dbp) *
    visual(QQNorm, qqline=:fitrobust, 
		markersize=5, color=:forestgreen)
	draw(plt, axis=(xlabel="Normal quantiles",
    	ylabel="DBP (mm Hg)"))
end

# ╔═╡ 69243e31-9eb3-4db7-a6d1-399e78893c11
md"The next two plots, shows the use of log scales."

# ╔═╡ 12de3413-4d46-4688-83e5-48f2b5bcc333
let 
...
# ╔═╡ 57b0fc4d-cdb3-4f0e-96ac-09855a98fab8
md"An alternative to log-scales, is to log-transform the variable, in this case, the scale of the axis needs to also be adjusted."

# ╔═╡ 97bc3d7c-80ca-4e39-9f85-351f107d4a06
let 
	plt = data(wcgs) * mapping(:dbp => log10) *
    visual(QQNorm, qqline=:fitrobust, markersize=5)
...
# ╔═╡ b6b87cd2-c6ce-4a29-b158-b17c75f12b6c
md"""
...
# ╔═╡ e0ee3fee-2547-48a2-85b9-a2982294a2c7
data(kfm) *
mapping(
...
# ╔═╡ 0028fbb2-4657-41f2-bef4-c0d1fb40ac3a
md"""
!!! note
...
# ╔═╡ 1dacc9d2-cae3-43cf-9126-aff8da38df75
let
	layers = visual(Scatter, markersize=8) + linear()
	data(kfm) *
	mapping(
		:mat_weight => "Maternal weight (kg)",
    	:dl_milk => "Breast-milk intake (dl/day)"
...
# ╔═╡ fc8ee199-04bd-4a5c-ae87-98f2b1dbf928
md"In the next plot, instead of adding a linear fit, we show the trend with a smoother:"

# ╔═╡ a88c6901-b825-4f9f-b5b2-e4b928ddffdb
let
...
# ╔═╡ a74babf4-5d2f-4474-bd4e-e4b284e4b939
md"In the next plot, we use different line colours and different symbols (by sex):"

# ╔═╡ 6b2397cf-14f6-45ca-81bf-0a940d7ad0a1
let
	layers = visual(Scatter, markersize=8) + linear()
	data(kfm) *
	mapping(
...
# ╔═╡ 8ba26d88-5e3f-46d5-af40-4a8a260eedcd
md"""
## Corner Plots
...
# ╔═╡ 075400c2-61c8-40dd-a2cb-bf1e0a9bca3d
let
	nms = names(iris, 1:4)
...
# ╔═╡ fc8c1348-a91f-4f5e-b278-aff5aa2cc9d9
let
	nms = names(iris, 1:4)
	p = select(iris, nms .=> replace.(nms, "_" => " "))
	pairplot(p => (
...
# ╔═╡ ee998b15-f533-4599-9795-1f926fe58443
md"Let's take a look at potential relationships between ozone concentrations in the air (pollution) and some environmental variables."

# ╔═╡ 0e72e2e2-e7cd-44b1-88b5-f764d978c13b
let
	nms = names(air, 1:4)
	p = select(air, nms)
	pairplot(p,
...
# ╔═╡ 2b682705-1172-49b5-9a16-89d3599e69e9
let
	nms = names(air, 1:4)
	p = select(air, nms)
	pairplot(
...
# ╔═╡ ff1188ae-ee80-497f-b134-0e697c2954c8
md"From the previous plot we can see that the strongest negative correlation is between ozone and wind. Let's take a further look."

# ╔═╡ 82f61274-f2db-4924-b889-67d99eca33ab
let
	d = data(air) * mapping(:Wind, :Ozone)
	p1 = d * visual(Scatter, markersize=8) * mapping(color=:Month)
	p2 = d * smooth()
	draw(p1 + p2,
		axis=(
		xlabel="Wind (mph)",
...
# ╔═╡ 7d991031-4be3-43ad-ba95-2419f1629d61
md"""
## Line Charts
...
# ╔═╡ 2ece1112-10cb-4975-be43-c653093ffd76
let 
...
# ╔═╡ 59a3ac00-74b5-4be8-9f66-009436f08aac
md"Reshaping *wide* to *long* data:"

# ╔═╡ 8a41f815-9e31-42ae-9764-2cdf06cbe1ab
flu_melt = @pivot_longer(flu, !week); flu_melt |> head

# ╔═╡ 95a2a831-8610-48d4-b53d-a76b90151e5c
data(flu_melt) *
mapping(
	:week => "Date",
...
visual(ScatterLines, markersize=8) |>
draw

# ╔═╡ cb7f6cae-bd3b-4347-a70d-86def0190224
md"""
# Comparing Groups
...
# ╔═╡ e0c8094a-b894-4c12-a865-04c2a919ff3f
data(@filter(wcgs, !ismissing(chol))) *
mapping(
	:chd => "CHD",
    :chol => "Cholesterol (mg/dl)"
...
# ╔═╡ a5d1d105-c7e7-4b75-856b-646359e95282
md"In the previous plot the presence of an outlier is clear. If we would like to remove that outlier, we would need to declare that in the report. For demonstration purposes, if we do not want to show the outlier in the plot, we have the option to *filter* the data using the `@subset` command."

# ╔═╡ 5b191f24-d03f-4c54-90fc-4c0670799712
data(@filter(wcgs, !ismissing(chol), chol < 500)) *
mapping(
	:chd => "CHD",
    :chol => "Cholesterol (mg/dl)"
) *
visual(BoxPlot, color=:plum) |>
draw

# ╔═╡ 186d4a28-e05e-42a9-9439-62a2f75c4862
data(birth) *
mapping(
	:race => "Ethnicity",
    :bwt => "Birth weight (g)",
...
# ╔═╡ 6dd5298e-514c-4370-996a-c71c69f6cb73
md"Faceting:"

# ╔═╡ f874366c-7a91-4c26-ae2e-5e79b3ae9618
data(birth) *
mapping(
	:Race => "",
    :bwt => "Birth weight (g)",
...
# ╔═╡ 27484c02-a8f8-45c9-9ac2-8a0b095fe5ae
md"""
...
# ╔═╡ 3861f11b-a633-4502-9bc1-1e6cf1e7452e
data(energy) *
mapping(
	:stature => "Stature", 
    :expend => "Energy expenditure (MJ)",
) *
visual(RainClouds, clouds=nothing, plot_boxplots=false, markersize=7, ) |>
draw

# ╔═╡ a339cd1c-d6c0-4324-99e0-132d187095d5
md"""
Umm, not perfect, dots are plotted to the right side of the corresponding thick. An alternative is to use rain clouds: we show the distribution on the left side (e.g., a violin plot) and the observations on the right side (i.e., a strip chart).
"""

# ╔═╡ adb3c339-7c27-4a8c-a4c3-e31f321c79ac
data(energy) *
mapping(
	:stature => "Stature", 
    :expend => "Energy expenditure (MJ)",
...
# ╔═╡ 768c4a2f-27d9-4814-bf7d-3de782d49a16
md"""
We can use a function from package `pubh` to generate a data frame with bootstrap confidence intervals around the mean, stratified by a categorical variable. With this data frame, then we can construct a strip chart with error bars as shown bellow.
"""

# ╔═╡ 43f6454c-4161-4949-9815-dbfc611f2005
let
...
# ╔═╡ 181a219e-daff-4c2b-b4cb-96d51b80ce3b
md"""
!!! note
	...
	5. In the `Axis` section, we are using specific, defined labels as the ticks for the categorical variable, using its corresponding levels.
"""

# ╔═╡ 49d20130-2282-4428-b8f0-5ac9b06daac2
data(birth) *
mapping(
...
visual(
	...
	plot_boxplots=false, 
	markersize=5, jitter_width=0.1
) |>
draw

# ╔═╡ f9dc99f3-d127-4d02-ac09-e6805408843e
md"""
## Violin Plots
...
# ╔═╡ bf64dd1b-7267-47ed-8a2a-0b9917cea9c7
data(kfm) *
mapping(
    :sex => "",
    :dl_milk => "Breast-milk intake (dl/day)",
...
# ╔═╡ cdecce6c-2920-4bb1-a184-a0e0aa4e5a2a
data(wcgs) *
mapping(
    :chd => "Coronary Heart Disease",
    :sbp => "SBP (mm Hg)",
    color = :beh_pat => "Behaviour \n Pattern",
    dodge = :beh_pat
) *
visual(Violin, datalimits=extrema) |>
draw

# ╔═╡ 1f8f6618-6fd5-44b4-8f5b-d7daf281b85b
data(wcgs) *
mapping(
...
# ╔═╡ 88d8e1dc-09be-4653-b1bc-aa7f2a5b3d95
md"""
## Bar Charts
...
# ╔═╡ 5b35def6-1f52-4e39-9d33-d3468541a688
data(birth) *
mapping(
    :race => "Ethnicity of Mother",
    color=:low => "Low birth weight?",
    dodge=:low
) *
frequency() |>
draw

# ╔═╡ 8da547ef-a898-4121-b899-b0d383febeae
md"""
### Expectation
For continuous variables, we can plot the expected (mean) values, with `expectation`.
"""

# ╔═╡ fdbef3d3-baad-43d1-866e-bc30301bd9a7
data(birth) *
mapping(
  :race => "Ethnicity of Mother",
  :bwt => "Birth weight (g)",
...
# ╔═╡ 80371a37-2f1c-450a-b58b-3e35e3821432
data(birth) *
mapping(
  :Race,
  :bwt => "Birth weight (g)",
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "6d97a1233aa9ef6c6131a3e9d8baea523cb103d1"
