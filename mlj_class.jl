...
# ╔═╡ fafef7b3-8eba-49f6-87a5-6b76243216c5
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ 91b437b9-cd33-4cb9-87de-d4e69bf4787c
begin
	using DataFrames, DataFrameMacros, RCall, MLJ
	using StatsBase, StableRNGs, CategoricalArrays
	using MLJ: schema, predict
end

# ╔═╡ 336bf3dd-708e-4825-a307-a0820bd382f4
begin
	using StatsPlots, PlotThemes, TreeRecipe
	theme(:wong)
end

# ╔═╡ 909d507e-cf5d-47f1-8860-e7a621e6b58a
using MLJDecisionTreeInterface

# ╔═╡ 627241c3-b98b-42cf-9d4e-c780c30448b6
using MLJMultivariateStatsInterface

# ╔═╡ 96f4c9f0-3575-49cc-ae8a-dbfd78cfeb9f
include("pubh.jl");

# ╔═╡ 78e69eea-c566-11ee-2441-bb59bbf52bb7
md"""
...
# ╔═╡ 5f346bd4-ca56-4b1a-b8ec-5177679218f9
md"""
## [📖 Main Menu](index.html)
"""

# ╔═╡ db9e6e13-7c1b-4ce5-8d6f-61f792227528
begin
	import DecisionTree
	DT = DecisionTree
end;

# ╔═╡ a06e596c-9dad-4626-8f31-cc99ae67fcdf
md"""
# Data 
"""

# ╔═╡ e2b01ab4-c0e0-4faa-8d97-03ec96cbefa8
iris = rcopy(R"datasets::iris"); iris |> schema

# ╔═╡ f0772c3c-9df1-4df8-8502-9ddfe92abe92
md"""
## Data splitting

We `unpack` the data to define the *response* and *explanatory* variables:
"""

# ╔═╡ 859a59da-ca33-450f-b8d4-de392fbd0e93
y, X = unpack(iris, ==(:Species)); 

# ╔═╡ 80f08b2e-86c4-4a11-a54c-df315ea84496
begin
	rng = StableRNG(566)
...
# ╔═╡ 70815423-9969-4fb2-86a8-f825063c2f07
md"""
...
# ╔═╡ bb90ed7c-8fda-4ee7-aecb-edc363accd7c
dtc = @load DecisionTreeClassifier pkg=DecisionTree verbosity=0;

# ╔═╡ 3a3fedf1-b620-46fe-998f-9dcad92ad5b7
pruned_model = dtc(max_depth=2)

# ╔═╡ 999bd064-e516-43d9-bd87-8484afc02f3f
pruned_mach = machine(pruned_model, X, y);

# ╔═╡ 88a5c7ac-5712-4831-bc89-af14969624d9
# ╠═╡ show_logs = false
fit!(pruned_mach, rows=train);

# ╔═╡ ac7523f3-6876-4c83-93b8-b6e06c30c11f
fitted_params(pruned_mach).tree

# ╔═╡ e2275017-debe-43e4-ba03-769f7327b0a8
plot(fitted_params(pruned_mach).tree)

# ╔═╡ d20f0cf3-7fda-4f0e-ae47-69a02d4afd95
pruned_mach |> feature_importances

# ╔═╡ 3ecca619-ea28-4d50-9278-f833dee84ff8
md"""
## Performance
"""

# ╔═╡ d303c1ac-d035-4fdf-9a58-99a01a876055
pruned_ŷ = predict(pruned_mach, rows=test);

# ╔═╡ 6e874419-a2eb-45b3-a2ab-f20ca4e69b0d
pruned_ȳ = predict_mode(pruned_mach, rows=test);

# ╔═╡ 589ca594-e0e6-4c93-96d1-b80d07a3d9bb
cross_entropy(pruned_ŷ, y[test]) |> mean |> r3

# ╔═╡ da183fe8-fde4-4864-89ec-4c50297c9fda
misclassification_rate(pruned_ȳ, y[test]) |> r3

# ╔═╡ b77cded2-2d76-444f-ad6c-4b327ee9bfdf
accuracy(pruned_ȳ, y[test]) |> r3

# ╔═╡ 04cd6bb5-c212-4c0c-850e-29c3cb4a5635
kappa(pruned_ȳ, y[test]) |> r3

# ╔═╡ d87c7f44-2ea3-4978-990f-ad9c61564e73
md"""
...
# ╔═╡ 964602fc-cc73-480b-9d8c-e5b3ff775c85
dt_model = dtc(merge_purity_threshold=0.9)

# ╔═╡ 434e463f-18b4-45e6-8147-8bbf5606ad1d
dt_mach = machine(dt_model, X, y);

# ╔═╡ a16d79d7-31ba-4e69-a071-717de92c74dd
# ╠═╡ show_logs = false
fit!(dt_mach, rows=train);

# ╔═╡ e8def537-113f-4e11-a91c-7b9c0fc073c8
fitted_params(dt_mach).tree

# ╔═╡ d08bb3a3-5a1b-4a00-8bd1-cdeaf8049008
plot(fitted_params(dt_mach).tree)

# ╔═╡ d387f06f-796d-4e36-b976-76d92ab25949
md"""
## Performance
"""

# ╔═╡ 23067083-3f8c-4169-b345-d937d9fda5f4
dt_ŷ = predict(dt_mach, rows=test);

# ╔═╡ f375e280-86b0-44e5-9045-ec99fd7d36bb
dt_ȳ = predict_mode(dt_mach, rows=test);

# ╔═╡ 67ad8eca-bd5a-4d88-97c3-3a780ac51baa
cross_entropy(dt_ŷ, y[test]) |> mean |> r3

# ╔═╡ cb5499f1-ad7b-47d5-99ae-00188ab33abe
misclassification_rate(dt_ȳ, y[test]) |> r3

# ╔═╡ fa5e87b4-c19b-43b2-b375-bd21451b41ec
accuracy(dt_ȳ, y[test]) |> r3

# ╔═╡ 0e342880-c663-4c49-855b-b33e7b22f334
kappa(dt_ȳ, y[test]) |> r3

# ╔═╡ 5a09ea37-83a7-4729-94f1-3e680e22e4b1
md"""
# Random Forest
...
# ╔═╡ daae1693-bfe4-45c1-aff1-ff02b7e1675f
Forest = @load RandomForestClassifier pkg=DecisionTree verbosity=0;

# ╔═╡ 73896640-9d89-4a89-b371-f3439aae5bfd
md"""
Following the example from `DecisionTree`, we use particular parameters for the model.
"""

# ╔═╡ e9165068-cee7-460d-a4af-9af104794e99
forest = Forest(
  	n_subfeatures = 2,
  	n_trees = 10,
...
# ╔═╡ df808c8a-28bf-4977-8394-11f9898339d7
forest_mach = machine(forest, X, y);

# ╔═╡ 0f2b03fd-c51d-4cf9-9926-ef1abee74b97
# ╠═╡ show_logs = false
fit!(forest_mach, rows=train);

# ╔═╡ d49e4151-4212-401d-95e5-d62d21741abe
fitted_params(forest_mach)

# ╔═╡ a4311363-6c50-45fd-b5d7-8f35aa2bd624
forest_mach |> feature_importances

# ╔═╡ db99fc7b-9620-4cd0-8bf0-17b8b213281a
md"""
## Performance
"""

# ╔═╡ 7d1ab9db-b30c-4555-8328-cd6df22c96cc
forest_ŷ = predict(forest_mach, rows=test);

# ╔═╡ eac19e7a-248d-480b-b42d-da2c68ae8f23
forest_ȳ = predict_mode(forest_mach, rows=test);

# ╔═╡ 9ab81126-2a5b-4b61-aaf6-e3fc0d8a4423
cross_entropy(forest_ŷ, y[test]) |> mean |> r3

# ╔═╡ bb9e9e30-3234-472e-845e-f89a68dbf148
misclassification_rate(forest_ȳ, y[test]) |> r3

# ╔═╡ d8e64a9a-ce3b-48a2-911d-d50ae2614e16
accuracy(forest_ȳ, y[test]) |> r3

# ╔═╡ fa07461b-51e3-45e0-a6e1-672ac4810127
kappa(forest_ȳ, y[test]) |> r3

# ╔═╡ dd99c83b-2785-49de-9f0d-419a4bca6a5e
md"""
...
# ╔═╡ 472266c9-702c-4ad0-a0ef-d8cc080315a6
LDA = @load LDA pkg=MultivariateStats verbosity=0;

# ╔═╡ 4ba2c65e-e05b-45f6-b968-87a277e224a3
lda_model = LDA()

# ╔═╡ 285a9931-580f-47ec-a5e8-3e2799ff8824
lda_mach = machine(lda_model, X, y);

# ╔═╡ dd09e57c-d543-48bd-85a5-79fa08f1d4df
# ╠═╡ show_logs = false
fit!(lda_mach, rows=train);

# ╔═╡ 9a1540ab-922b-4f58-a763-da16fa82b09c
fitted_params(lda_mach)

# ╔═╡ b3c3c17a-27da-4bdd-b8be-a28b6e8e2dca
md"""
## Visualisation
"""

# ╔═╡ 414a8ab5-ff49-4b6b-89ba-dfc307f192b7
fitted_params(lda_mach).projection_matrix |> size

# ╔═╡ 9e9fde67-7615-4b5f-be1d-47150e9120c1
lda_x = MLJ.transform(lda_mach); lda_x |> size

# ╔═╡ 526bd276-ef81-4548-b314-1cf72fbcd3b2
lda_x |> head

# ╔═╡ 7e25cac7-43b0-4f89-b5b3-c4210f73bf2a
lda_df = DataFrame(
	PC1 = lda_x.x1,
	PC2 = lda_x.x2,
	Species = string.(y)
...
# ╔═╡ e94c35d8-1537-41cb-b0c5-f18e32b4c80e
@df lda_df scatter(
...
# ╔═╡ 438f5b7e-ef74-49b1-a648-81b8cdc3fb75
md"""
## Performance
"""

# ╔═╡ 551ecca5-16e4-4314-a878-b0d7d39dc45c
lda_ŷ = predict(lda_mach, rows=test);

# ╔═╡ c1e3d45b-3ccd-4d2a-a6bb-586cea27addb
lda_ȳ = predict_mode(lda_mach, rows=test);

# ╔═╡ 08f7c924-7495-423e-a698-ea4dcae62ad8
cross_entropy(lda_ŷ, y[test]) |> mean |> r3

# ╔═╡ 77eb0283-984d-4f2b-b697-1366d5953234
misclassification_rate(lda_ȳ, y[test]) |> r3

# ╔═╡ 50abb3d7-81e2-4ea7-9cef-98bf69fb95b4
accuracy(lda_ȳ, y[test]) |> r3

# ╔═╡ 33f15f2c-ff48-4b36-a944-b98e9a2ebcf5
...
manifest_format = "2.0"
project_hash = "8fdcc8b83dd9a46e571b0c19b749b23800c853ec"
