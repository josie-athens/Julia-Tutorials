...
# ╔═╡ 948f95c7-91dd-4edb-a655-421dbfa2253b
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ ae8cd568-6dd0-4d24-ad84-b842ad102ac4
begin
...
# ╔═╡ dca6d9ab-1358-47d7-99a8-7062c0f3e4a8
begin
...
# ╔═╡ 84d81dce-1534-11ee-24d4-93a1552aacaf
md"""
# An Intro to `DifferentialEquations`

!!! note \"Chris Rackauckas\"

	- Adapted by: **Josie Athens**
	- 19 October 2023
"""

# ╔═╡ 74a18bf6-353a-499c-9970-b44d6ecfe3f6
md"""
# 📖 Main Menu
...
# ╔═╡ 9cb920d5-2ccf-424d-b8f9-c2726f49df5e
md"""
...
# ╔═╡ 59ad0528-b38a-4ed6-8470-210c7170cf3b
md"""
## Background
...
# ╔═╡ fab7cb6a-0b2d-468c-8ca7-d693e598eae7
md"""
...
# ╔═╡ e654d442-288c-4351-886f-8fe91f8ff172
begin
	f(u,p,t) = 0.98u
	u0 = 1.0
	tspan = (0.0, 1.0)
...
# ╔═╡ 126245ab-caf9-4c38-ba55-a221dde198cf
md"To solve our `ODEProblem` we use the command `solve`."

# ╔═╡ 7535ed6c-0c7a-44fd-b8eb-b888e1ddaaf1
sol = solve(prob)

# ╔═╡ 1b17a3c7-1523-491b-ae47-bc5c360a6670
md"and that's it: we have succesfully solved our first ODE!"

# ╔═╡ bb0efa46-b6a7-49e2-9f14-10d5f2701a23
md"""
## Analyzing the Solution

Of course, the solution type is not interesting in and of itself. We want to understand the solution! The documentation page which explains in detail the functions for analyzing the solution is the [Solution Handling](https://docs.sciml.ai/dev/basics/solution/) page. Here we will describe some of the basics. We can plot the solution using the plot recipe provided by `Makie` or by generating a dataframe and use `AlgebraOfGraphics`.
"""

# ╔═╡ a42d2598-4d5e-4f1b-b606-8c246ba0756a
sol_df = DataFrame(sol)

# ╔═╡ 4999a381-d4b6-4700-bdc4-08d0c45c8648
data(sol_df) *
mapping(
	:timestamp => "Time (t)",
	:value => "u(t) (in μm)"
) *
visual(Lines, color=:firebrick, linewidth=3) |>
draw

# ╔═╡ 75661f8b-7f77-46df-84df-f7e319a2d9b2
let
	f = Figure()
	Axis(f[1, 1], xlabel = "Time (t)", ylabel = "u(t) (in μm)")

	lines!(sol, linewidth=3)
	f
end

# ╔═╡ 88565142-4d3e-4291-8ad5-c89f4d8e4526
md"""
From the figures above, we see that the solution is an exponential curve, which matches our intuition.

We can add other pieces to our plot. For this ODE we know that the true solution is $u(t) = u_0 exp(at)$, so let's add some of the true solution to our plot:
"""

# ╔═╡ 6fe20ddd-75ba-4b04-938b-b2fe7cb5286d
let
	f = Figure()
	Axis(f[1, 1], xlabel = "Time (t)", ylabel = "u(t) (in μm)")

	lin1=lines!(sol.t, sol.u, linewidth=3)
	lin2=lines!(sol.t, t->1.0*exp(0.98t), linewidth=2, 
		linestyle=:dash, color=:firebrick)
	Legend(f[1, 2],
		[lin1, lin2],
		["ODE solution", "True solution"]
	)
	f
end

# ╔═╡ 5fcd7b37-a941-45a1-badd-63313368e183
md"We get the array of time points with `sol.t` and the array of solutions with `sol.u`. So `sol.u[i]` is the solution at time `sol.t[i]`."

# ╔═╡ bb466cf5-a866-4cd9-a62f-d6438bb3c992
sol_df.expected = exp.(0.98sol.t);

# ╔═╡ f861e288-7c99-49e6-9fdb-c6b3012c2b21
let
	labels = ["ODE output", "True solution"]
	plt = data(sol_df) *
	mapping(
		:timestamp,
		[:value, :expected],
		color=dims(1) => renamer(labels) => "Solutions",
		linestyle=dims(1) => renamer(labels) => "Solutions"
	) *
	visual(Lines, linewidth=2)
	draw(plt, 
		axis=(xlabel = "Time (t)", ylabel = "u(t) (in μm)"))
end

# ╔═╡ 8a479394-09e1-443e-89e7-282316b904db
md"We can compute arrays of functions of the solution values using standard comprehensions, like:"

# ╔═╡ 457b6a73-f8f4-4970-9a13-0c8034fa0d52
[t+u for (u,t) in tuples(sol)]

# ╔═╡ 6e2eb8e5-b69b-4077-8634-669786e1c034
md"However, one interesting feature is that, by default, the solution is a continuous function. If we check the print out again:"

# ╔═╡ c129eeab-a5d8-4720-bf80-55e0e120b441
sol

# ╔═╡ 11a3e9c1-c266-4f76-a492-7d24ead2cf85
md"""
The solution has an order changing interpolation. The default algorithm automatically switches between methods in order to handle all types of problems. For non-stiff equations (like the one we are solving), it is a continuous function of 4th order accuracy. We can call the solution as a function of time `sol(t)`. For example, to get the value at `t=0.45`, we can use the command:
"""

# ╔═╡ 6bd317ac-2c60-4078-83d8-5a520224d608
sol(0.45)

# ╔═╡ 8bd2a16b-6547-43a9-b8d4-58644c4a774c
md"""
### Controlling the Solver

`DifferentialEquations` has a common set of solver controls among its algorithms which can be found [at the Common Solver Options](https://docs.sciml.ai/dev/basics/common_solver_opts/) page. We will detail some of the most widely used options.
...
# ╔═╡ 8d2d471e-d6e5-4af9-90a1-a6ebd1a1e004
sol2 = solve(prob, abstol=1e-8, reltol=1e-8)

# ╔═╡ 1e68735f-b88f-4e5c-bb70-17f79dc379a8
let
	f = Figure()
	Axis(f[1, 1], xlabel = "Time (t)", ylabel = "u(t) (in μm)")

	lin1=lines!(sol2.t, sol2.u, linewidth=3)
	lin2=lines!(sol2.t, t->1.0*exp(0.98t), linewidth=2, 
		linestyle=:dash, color=:firebrick)
	Legend(f[1, 2],
		[lin1, lin2],
		["ODE solution", "True solution"]
...
# ╔═╡ 356c0f33-094e-472c-b948-5f394f9c75cc
md"""
...
# ╔═╡ 00503303-1eb5-4f8a-9052-8cf16224720f
sol3 = solve(prob, saveat=0.1)

# ╔═╡ 1f360ffc-e67f-4e9d-b287-86a694851dca
md"If we want to solve and only save the final value, we can even set `save_start=false`."

# ╔═╡ 5bf0f76f-1c16-4e8c-bb9a-2c3205ff74f3
sol_final = solve(prob, save_everystep=false, save_start = false)

# ╔═╡ 2dd9680b-8776-44d9-abd3-c4dc294f4a46
md"""
...
# ╔═╡ ba05eb77-05b5-4196-9776-a30e42884f7c
md"""
...
# ╔═╡ 573fef95-af1f-4911-9366-960e64344d2c
sol_stiff = solve(prob, alg_hints=[:stiff])

# ╔═╡ 345fc3b6-887f-4aeb-8905-d78ddda245dc
md"""
...
# ╔═╡ 235476ef-4112-4c4f-8b0c-a2241a6f270f
sol_tsit = solve(prob,Tsit5(), reltol=1e-6)

# ╔═╡ a70bb727-bf73-4eed-b16b-2e89800efdf9
md"""
...
To define a system of differential equations in DifferentialEquations, we define our `f` as a vector function with a vector initial condition. Thus, for the vector `u = [x,y,z]'`, we have the derivative function:
"""

# ╔═╡ 2617ec02-c518-484a-94fb-419437ad9d98
...
# ╔═╡ 3f552918-045b-4871-95f5-024d184d3159
md"""
...
# ╔═╡ bdfb8d08-b998-4246-8fa1-5584b7876fef
x0 = [1.0,0.0,0.0]

# ╔═╡ 04521d8b-f091-46fe-bcae-abcd87ed9dd0
md"""
...
# ╔═╡ 972b31e9-7f5a-4ef5-a30d-a6c2019b3fa0
begin
	p = (10,28,8/3)
	lor_tspan = (0.0,100.0)
	lor_prob = ODEProblem(lorenz!, x0, lor_tspan, p)
end

# ╔═╡ 2ca4a100-13d3-4e7e-95c2-0d43e7cd6297
md"Now, just as before, we solve the problem:"

# ╔═╡ a2f41df0-09e2-45da-b4c0-35c64072f25e
lor_sol = solve(lor_prob)

# ╔═╡ b1fbe98e-0cc3-4fa1-b0b8-05a54493ff58
md"""
The same solution handling features apply to this case. Thus `lor_sol.t` stores the time points and `lor_sol.u` is an array storing the solution at the corresponding time points.

However, there are a few extra features which are good to know when dealing with systems of equations. First of all, `lor_sol` also acts like an array. `lor_sol[i]` returns the solution at the `i`th time point.
"""

# ╔═╡ 11f4c201-c5c1-472a-a494-4d06971667ea
lor_sol.t[10], lor_sol[10]

# ╔═╡ b461e890-05a4-4d53-97f4-ee947e342825
md"Additionally, the solution acts like a matrix where `lor_sol[j,i]` is the value of the `j`th variable at time `i`:"

# ╔═╡ 45af066a-da6b-451f-8ff0-af5cff129cd5
lor_sol[2, 10]

# ╔═╡ 206e1d85-7ed2-444e-aa7f-dc729219d79c
md"We can get a real matrix by performing a conversion:"

# ╔═╡ d64f4068-3128-4ec6-b81a-b972af59b211
lor_mat = Matrix(lor_sol)

# ╔═╡ 39d0522a-6c51-4348-aa72-cca59bf35972
md"We will use `Plots` for plotting in 3D." 

# ╔═╡ 9284ba3c-846d-43e1-9cff-8130b9251687
begin
	import Plots
	Plots.plotly(size=(400, 300));
end

# ╔═╡ e04294cc-00b5-4dde-aaab-fa22276f96ed
let
	...
	draw(plt, 
		axis=(xlabel = "Time (t)", ylabel = "x(t)"))
end

# ╔═╡ a13b5121-8f20-4f9d-b579-54c30f6d1c7c
Plots.plot(lor_sol)

# ╔═╡ c0e2b1a4-56b3-4aa7-a1d5-3d20ea29d933
md"If we instead want to plot values against each other, we can use the `vars` command. Let's plot variable `1` against variable `2` against variable `3`:"

# ╔═╡ f2be1314-3705-4ab4-8648-e12d49b77f77
lor_df = DataFrame(lor_sol)

# ╔═╡ 6087dec0-83a8-42b3-b747-1d74310798e9
meshscatter(lor_df.value1, lor_df.value2, lor_df.value3, 
	markersize = 0.3, rotations=0.3)

# ╔═╡ ed9795d3-2958-49c9-831c-1b73f480c229
Plots.plot(lor_sol, idxs=(1,2,3))

# ╔═╡ e272cfc6-b09f-40a3-a6ab-198c5ec1657f
md"This is the classic Lorenz attractor plot, where the `x` axis is `u[1]`, the `y` axis is `u[2]`, and the `z` axis is `u[3]`. Note that the plot recipe by default uses the interpolation, but we can turn this off:"

# ╔═╡ 2cadaefe-c74c-4505-a914-ea72ebd0d3f2
Plots.plot(lor_sol, idxs=(1,2,3), dense=false)

# ╔═╡ 6a73e227-f44a-4afa-a522-7b4252f38b7d
md"Yikes! This shows how calculating the continuous solution has saved a lot of computational effort by computing only a sparse solution and filling in the values! Note that in idxs, `0=time`, and thus we can plot the time series of a single component like:"

# ╔═╡ 5844c01c-cc1e-4f20-a5ee-26051e3dd340
Plots.plot(lor_sol, idxs=(0,2))

# ╔═╡ bf2fe3fa-4b34-4fc9-884e-5b6f8255d828
data(lor_df) *
mapping(
	:timestamp => "Time (t)",
	:value2 => "y(t)"
) *
visual(Lines, color=:firebrick, linewidth=1) |>
draw

# ╔═╡ cdc0d5b4-8faf-468d-9976-976a1e80d01d
md"""
# Internal Types

The last basic user-interface feature to explore is the choice of types. `DifferentialEquations` respects your input types to determine the internal types that are used. Thus since in the previous cases, when we used `Float64` values for the initial condition, this meant that the internal values would be solved using `Float64`. We made sure that time was specified via `Float64` values, meaning that time steps would utilize 64-bit floats as well. But, by simply changing these types we can change what is used internally.

As a quick example, let's say we want to solve an ODE defined by a matrix. To do this, we can simply use a matrix as input.
"""

# ╔═╡ 7b625365-b53b-4e51-b684-ba56e58a1452
begin
	A  = [1. 0  0 -5
		  4 -2  4 -3
     	 -4  0  0  1
	...
	mat_u0 = rand(4,2)
	mat_tspan = (0.0,1.0)
	mat_f(mat_u, mat_p, t) = A*mat_u
	mat_prob = ODEProblem(mat_f, mat_u0, mat_tspan)
	mat_sol = solve(mat_prob);
end

# ╔═╡ 8b734430-9287-4746-9cc8-bf3aea17c51f
md"There is no real difference from what we did before, but now in this case `mat_u0` is a `4x2` matrix. Because of that, the solution at each time point is matrix:"

# ╔═╡ 2230e499-f4c2-433c-853d-f95d70dc2479
mat_sol[3]

# ╔═╡ a9987c77-b57f-4c83-85ef-857429af3057
md"In `DifferentialEquations`, we can use any type that defines `+`, `-`, `*`, `/`, and has an appropriate `norm`. For example, if we want arbitrary precision floating point numbers, we can change the input to be a matrix of `BigFloat`:"

# ╔═╡ 48197b89-1d11-4211-be07-df8daa4575b7
big_u0 = big.(mat_u0)

# ╔═╡ 42903061-59c2-4198-95c2-4bc8b4236af0
md"and we can solve the `ODEProblem` with arbitrary precision numbers by using that initial condition:"

# ╔═╡ 31343f55-2c08-4b80-8831-1bf20722630b
begin
	big_prob = ODEProblem(mat_f, big_u0, mat_tspan)
	big_sol = solve(big_prob);
end

# ╔═╡ 170279f6-f317-4446-bb2a-4a8940dce479
big_sol[1,3]

# ╔═╡ f731cb97-0c33-4a6f-89da-cdccf01bc59c
md"To really make use of this, we would want to change `abstol` and `reltol` to be small! Notice that the type for *time* is different than the type for the dependent variables, and this can be used to optimize the algorithm via keeping multiple precisions. We can convert time to be arbitrary precision as well by defining our time span with `BigFloat` variables:"

# ╔═╡ 91f6e82e-1c4a-4f33-ac7e-086d02c5d2db
begin
	Big_prob = ODEProblem(mat_f, big_u0, big.(mat_tspan))
	Big_sol = solve(Big_prob);
end

# ╔═╡ 9088f538-f8a8-40af-826e-179b4814b5ca
Plots.plot(Big_sol)

# ╔═╡ 8b3a9962-1c7b-4c93-b2de-5ff95c3c56d1
begin
	Big_df = DataFrame(Big_sol)
	DataFrames.rename!(
		Big_df,
		:value1 => :u₁,
		:value2 => :u₂,
...
# ╔═╡ f032e55a-3450-4c7f-8992-2e350b6e5020
Big_melt = stack(Big_df, Not(:timestamp)); first(Big_melt, 5)

# ╔═╡ ec860279-90bb-43e4-9a14-daa030f69b49
data(Big_melt) *
	mapping(
		:timestamp => "Time (t)",
		:value => "u(t)",
...
(smooth() + mapping()) * mapping(markersize=1) |>
draw

# ╔═╡ d862951f-aad3-4ca0-8f34-b5da5947b473
md"""
# Conclusion
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "dfa429282c0f8c2c62419d1fbac5f1a7dffed103"
