...
# ╔═╡ a63a45a6-59b0-4de8-8289-c25cb840dec9
using PlutoUI; TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ 56592548-f0c7-434e-a43e-27b1cb6a5b78
begin
	using StatsBase, DataFrames, DataFrameMacros
	using RCall, CategoricalArrays, TexTables, MLJ
	using SurvivalAnalysis, Survival
	import AnovaGLM as aov
	using MLJ: schema
end

# ╔═╡ f2d6b863-8ee7-4e39-86ef-81c795fc4f68
begin
	using StatsPlots, PlotThemes
	theme(:wong)
end

# ╔═╡ 3fb436fe-30cb-47e3-8438-ba49f41306d0
begin
	include("pubh.jl")
	@rimport readr
	@rimport pubh
...
# ╔═╡ 94e8defe-d1ba-11ee-0a69-5f423ea650a2
md"""
# Survival Analysis
...
# ╔═╡ 29458b66-856b-433b-a804-b0a7ece60ce6
md"""
## [📖 Main Menu](index.html)
"""

# ╔═╡ 2c07097b-dfc7-45f2-8ecf-0d4c93437ffc
md"""
# Kaplan-Meier

!!! tip \"Example\"

	We will use the data set bladder from the `R` package `survival`, about recurrences of bladder cancer. The variable `stop` records the recurrence or censoring time and the variable `event` specifies the status where 1 means recurrence and 0 means everything else (including death for any reason). We will change the name of the variable `stop`, to avoid confusion with other commands in `julia` and convert `rx` to a categorical variable.
"""

# ╔═╡ b4b1c8a3-0c18-471f-b3a7-29cdb6e7032a
bladder = rcopy(R"survival::bladder"); bladder |> schema

# ╔═╡ dcdcaf34-8430-4595-9555-710f94da8fed
tabulate(bladder, :rx)

# ╔═╡ c117f14d-7db4-4a97-a297-4b15d2c95977
@transform!(
  bladder,
  :times = :stop,
  :rx = :rx == 1 ? "Pyridoxine" : "Thiotepa",
  :event = :event == 1
);

# ╔═╡ 66283fac-0588-4a9a-901e-d7a6cbe03d49
coerce!(bladder, :rx => Multiclass); bladder |> schema

# ╔═╡ 0a0a5cc1-54a0-4171-a11f-197d6a4c4bf8
md"""
Let’s compare treatments:
"""

# ╔═╡ 52c31d4f-eec9-4b75-9884-49d869003d53
begin
	pyr = @subset(bladder, :rx == "Pyridoxine")
	thio = @subset(bladder, :rx == "Thiotepa")
end;

# ╔═╡ 0e19962d-a70d-4b5d-8921-cdd6c38c503d
pyr_surv = kaplan_meier(@formula(Srv(times, event) ~ 1), pyr)

# ╔═╡ 05d2b52f-342c-4178-89c2-3a992222f7cc
thio_surv = kaplan_meier(@formula(Srv(times, event) ~ 1), thio)
...
SurvivalAnalysis = "d1dd7d07-e5c2-418a-8652-e20add45d312"
...
manifest_format = "2.0"
project_hash = "116d344d1918c1fa8e0a6cf9b715951d54b4c86c"
...
[[deps.SurvivalAnalysis]]
deps = ["DataFrames", "Distributions", "LinearAlgebra", "NLSolversBase", "Optim", "PrettyTables", "RCall", "Random", "RecipesBase", "StatsBase", "StatsModels"]
git-tree-sha1 = "ac2ae15c2bb509c47beca8b98c76e4bc71e47e37"
