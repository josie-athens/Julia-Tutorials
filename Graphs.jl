...
# ╔═╡ 7e181075-98f8-4e72-b30b-ecb74ef8158f
md"""
...
# ╔═╡ 6b8148dd-c445-406f-b70c-b1e82b76e8ea
md"# Distributions

## Histograms"

# ╔═╡ 520987a8-1e65-44b8-a702-fbe4b59ba8e9
@df birth histogram(
	:bwt, bins = 30, norm = :pdf,
	xlabel = "Birth weight (g)",
	ylabel = "PDF",
	legend = false
)

# ╔═╡ 173c2258-8c0d-4362-aeec-87b4ca3e872d
md"""
...
# ╔═╡ 5397be0f-12ce-402e-9a37-e3819cc0e19d
let
	p1 = @df @subset(wcgs, :chd == "No CHD") histogram(
		:sbp, bins = 20, color = "#eb98b5", opacity = 0.5,
		norm = :pdf, leg = false,
		xlabel = "SBP (mm Hg)",
		ylabel = "PDF",
		title = "No CHD"
	...
	p2 = @df @subset(wcgs, :chd == "CHD") histogram(
		:sbp, bins = 20, color = "#330C73", opacity=0.5,
		norm = :pdf, leg = false,
		xlabel = "SBP (mm Hg)",
		ylabel = "PDF",
		title = "CHD"
...
# ╔═╡ 6bef1ccc-3b1c-4a65-b869-9eda6127baca
...
The best way to determine if a continuous variable is normally distributed or not is with quantile-quantile plots (QQ-plots). We plot the quantiles of our variable of interest against quantiles from the standard normal distribution (which has a mean $\mu$ and a standard deviation $\sigma$). This type of QQ-plots against the normal distribution are known as QQ-normal plots. If the variable is normally distributed, then a linear relationship will be observed.
"""

# ╔═╡ 00adfe2c-01a9-4fce-b7e0-c63a493eeedd
@df birth qqnorm(
	:bwt, qqline =:R,
...
# ╔═╡ e1316cea-442e-44cd-95c0-ef48a34c7da2
@df kfm qqnorm(
	:dl_milk,
	qqline = :R,
	xlabel = "Theoretical quantiles",
	ylabel = "Breast-milk intake (dl/day)",
	marker=(2, :red, stroke(0))
)
...
# ╔═╡ 17b95cb1-5761-4d77-bfee-4c22c1a0cddd
@df wcgs qqnorm(
	:dbp, qqline = :R,
	ylabel = "DBP (mm Hg)",
	marker=(2, :red, stroke(0))
...
md"""
We can either, use a semi-log scale or plot the log of DBP:
"""

# ╔═╡ 4047f4bc-c5fc-4592-94cf-28797d74b389
@df wcgs qqnorm(
	:dbp, qqline = :R,
	ylabel = "DBP (mm Hg)",
	marker=(2, :red, stroke(0)),
...
# ╔═╡ 3757db70-1707-4112-bda1-ebd436ce2548
@df wcgs qqnorm(
	log.(:dbp), qqline = :R,
	ylabel = "log (DBP)",
	marker=(2, :red, stroke(0))
...
begin
	@df kfm plot(
		:mat_weight, lw =2,
		GLM.predict(lm(@formula(dl_milk ~ mat_weight), kfm)),
		ylabel = "Breast-milk intake (dl/day)",
		xlabel = "Maternal weight (kg)",
		legend = false
	)
	@df kfm scatter!(:mat_weight, :dl_milk, marker=(3, stroke(0)))
end

# ╔═╡ 5412c6a1-99cf-4e6b-8839-a09e9156ac7d
begin
	@df kfm plot(
		:mat_weight, lw =2,
		GLM.predict(lm(@formula(dl_milk ~ mat_weight*sex), kfm)),
		ylabel = "Breast-milk intake (dl/day)",
		xlabel = "Maternal weight (kg)",
		legend = :outertopright, group = :sex
	)
	@df kfm scatter!(:mat_weight, :dl_milk, group=:sex, marker=(3, stroke(0)))
end
...
md"""
When we are working with more than two continuous variables and we want to look at potential correlations between them, we can construct a single plot, with all two-variable combinations displayed in diffeerent panels. These are known as scatter plot matrices or `correlation` plots.
"""
...
@df air |> dropmissing cornerplot(
	cols(1:4), grid = false,
	markersize = 2, compact = true
)

# ╔═╡ 45b2cfb0-9d01-4677-8bc4-8634307a19aa
md"""
## Line charts
...
# ╔═╡ 8db8f954-953e-421e-8a51-a4d711afcd33
@df flu plot(
	:week, [:child, :young, :mid, :old], lw=2, marker=(3, stroke(0)),
...
# ╔═╡ 8677c408-2240-4133-9faf-173b34739585
flu_melt = stack(flu, Not(:week)); flu_melt |> head

# ╔═╡ 0429cdc0-da09-4632-83da-4514214c4f49
@df flu_melt plot(
	:week, :value, lw=2, marker=(3, stroke(0)),
	group = :variable,
	ylabel = "Cases", xlabel = "Date",
	label = ["Child" "Young" "Mid" "Old"],
)
...
# ╔═╡ 5d3cf03e-7924-428d-9dfa-bfed8c9341e9
...
md"""
!!! note
	We used abreviations: `leg` for `legend` and `msize` for `markersize`
"""

# ╔═╡ 7082e011-fcfd-4485-a313-2a5abf33e032
md"""
...
@df @subset(wcgs |> dropmissing, :chol .< 500) boxplot(
	:chd, :chol, color=:indianred, opacity=0.7,
	xlabel = "Coronary Heart Disease",
	ylabel = "Cholesterol (mg/dl)",
	leg = false, msize = 2
)
...
@df birth groupedboxplot(
	:race, :bwt, group = :smoke, opacity=0.7,
	xlabel = "",
	ylabel = "Birth weight (g)",
	msize = 2, bar_width = 0.7
)
...
@df energy dotplot(
	...
	bar_width = 0.2,
	leg = false, marker=(2, stroke(0))
)

# ╔═╡ 8694df4d-70c0-495d-89f1-0aceadc1fc0e
@df birth dotplot(
	:smoke, :bwt, group = :race,
	ylabel = "Birth weight (g)",
	marker=(2, stroke(0)), leg = :top,
	bar_width = 0.2
)
...
let
	@df @subset(birth, :race .== "White") violin(
		:smoke, :bwt,
		ylabel = "Birth weight (g)",
		side = :left, linewidth = 0, label = "White"
	)
	@df @subset(birth, :race .== "African-American") violin!(
		:smoke, :bwt,
		side = :right, linewidth = 0, label = "African-American"
	)
end

# ╔═╡ a7cbe689-6f0e-4e65-a59e-ef2bdd560499
let
	@df @subset(birth, :race .== "White") violin(
		:smoke, :bwt,
		ylabel = "Birth weight (g)", bar_width=0.5,
		side = :left, linewidth = 0, label = "White"
	)
	@df @subset(birth, :race .== "African-American") violin!(
		:smoke, :bwt,  bar_width=0.5,
		side = :right, linewidth = 0, label = "African-American"
	)
	@df @subset(birth, :race .== "White") dotplot!(
		:smoke, :bwt,  bar_width=0.5,
		side = :left, marker=(2, :black,stroke(0)), label = ""
	)
	@df @subset(birth, :race .== "African-American") dotplot!(
		:smoke, :bwt,  bar_width=0.5,
		side = :right, marker=(2, :black, stroke(0)), label = ""
	)
end

# ╔═╡ e870dca0-39a0-4aaa-b852-05f3400703f0
birth.Race = recode(
	birth.race, 
	"White" => "White", 
...
# ╔═╡ 4a351714-91fb-43ae-8b0f-d746926fa16b
begin
	@df @subset(birth, :Race .== "White") violin(
		:smoke, :bwt,
		ylabel = "Birth weight (g)",
	...
	@df @subset(birth, :Race .== "Non-White") violin!(
		:smoke, :bwt,
		side = :right, linewidth = 0, label = "Non-White"
	)
	@df @subset(birth, :Race .== "White") dotplot!(
		:smoke, :bwt,
		side = :left, marker=(2, :black,stroke(0)), label = ""
	)
	@df @subset(birth, :Race .== "Non-White") dotplot!(
		:smoke, :bwt,
		side = :right, marker=(2, :black, stroke(0)), label = ""
	)
end

# ╔═╡ 4a45a332-1dbb-423d-9069-8d396faf6c6b
md"## Bar charts"

# ╔═╡ 63177c05-ba3e-426a-b29d-c1da9cb48335
@df birth groupedbar(
	:race, :bwt, group = :smoke,
	ylabel = "Birthweight (g)"
)
...
manifest_format = "2.0"
project_hash = "560f929a46aa85f494be7638c0b397d9ce0276ff"
