...
# ╔═╡ 6dc0e79d-fea2-4d75-9f41-0e07cf8305d1
using PlutoUI; PlutoUI.TableOfContents(aside=true, title="📚 Contents")

# ╔═╡ c1b0cedf-2b07-47ed-814e-757cc8cc803f
begin
	using StatsBase, DataFrameMacros, TexTables
	using RCall, DataFrames, Effects
	using GLM, CategoricalArrays, MultipleTesting
	using StatsPlots
	import AnovaGLM as aov
	using MLJ: schema
end

# ╔═╡ 124f270a-adc3-448d-b685-6087c41c4a06
begin
	include("pubh.jl")
	@rimport readr
	@rimport pubh
...
# ╔═╡ 24ea8f9c-c918-11ee-1cf0-7139139059f3
md"""
# Poisson Regression
...
# ╔═╡ 3f4c5dee-8377-4c05-b77d-fde2cf7208ca
md"""
## [📖 Main Menu](index.html)
"""

# ╔═╡ 8500293d-c225-4ba1-815a-b449d85649f4
# ╠═╡ show_logs = false
R"""
library(pubh)
library(sjlabelled)
...
# ╔═╡ eb491dd0-4250-46fe-91e2-2a12d36403b7
md"""
# Count Data

!!! tip \"Example\"

	We will use the `quine` data set. These data come from a sociological study of Australian aboriginal and white children. The sample included children from four age groups (final year in primary school and first three years in secondary school who were classified as slow or average learners. The number of days absent from school during the school year was recorded for each child.
"""

# ╔═╡ 9f166821-f1a6-4f18-ae7e-bbae7f0d3c26
quine = rcopy(R"MASS::quine"); quine |> schema

# ╔═╡ c594ab78-dfc0-4c9f-94ac-fafb25d17185
md"""
We start by changing the levels of `Eth` to *Aboriginal* and *White* and the levels of `Sex` to *Male* and *Female*.
"""

# ╔═╡ 99a31094-b24f-4777-b950-5dff63d7d4b2
quine.Eth = recode(
	quine.Eth,
	"A" => "Aboriginal",
	"N" => "White"
...
# ╔═╡ 70138e8f-53e0-4bb7-bc61-ed6ef9eed662
quine.Sex = recode(
	quine.Sex,
	"F" => "Female",
	"M" => "Male"
...
# ╔═╡ 265bb4d5-9756-438f-9b9b-171447705c58
md"""
Descriptive statistics by ethnicity:
"""

# ╔═╡ d333040f-612d-4397-8da8-ef2402f29f5c
R"""
quine = $quine
...
quine |> 
  	mutate(
    	Eth = relevel(Eth, ref = "Aboriginal")
  	) |> copy_labels(quine) |> 
  	cross_tbl(by = "Eth") |> 
  	theme_pubh(2) |> 
...
# ╔═╡ d1cc574a-6045-4537-8150-f6630cda53a8
md"""
## Intercept only model
"""

# ╔═╡ ee4b6712-3edc-4d35-8ceb-3fa5e88ff73c
model_0 = glm(@formula(Days ~ 1), quine, Poisson());

# ╔═╡ 7471c42b-5710-4722-a614-644917394ba4
glm_coef(model_0 |> coeftable |> DataFrame)

# ╔═╡ bddb210c-f24d-4f32-89d2-fbbe6331ff6e
md"""
!!! note \"Question\"

	What does the coefficient in the above Poisson regression model represent?
"""

# ╔═╡ 27bbc2ba-333a-4e17-866b-2fe6b8c38f92
md"""
!!! hint \"Answer\"

	Represents the mean number of absent days. The mean number of absent days, regardless of ethnicity is 16.46 days (95% CI: 15.81, 17.13 days).
"""

# ╔═╡ fc936e95-5cd8-43e2-bde2-4856e16d22ea
md"""
## Univariate model

Model estimating the effect of ethnicity on number of absent days:
"""

# ╔═╡ 27a74b34-950c-45cd-8c3c-88beb37a3d7c
levels!(quine.Eth, ["White", "Aboriginal"]);

# ╔═╡ b749c126-fd17-4e8a-a8fd-3380c5cba19a
model_1 = glm(@formula(Days ~ Eth), quine, Poisson());

# ╔═╡ bef90f47-efea-4d97-862e-88ba440c4a39
glm_coef(model_1 |> coeftable |> DataFrame)

# ╔═╡ c88987aa-fff3-4091-82a3-9e9b387e8084
md"""
!!! note \"Question\"
...
# ╔═╡ 556548de-a45d-4477-b61b-d4610589308d
md"""
!!! hint \"Answer\"

	On average, white students miss about 12 days per scholar year. Students from aboriginal origin miss about 74% more number of days per school-year (IRR = 1.74 absences/school-year, 95% CIs 1.61, 1.89; p < 0.001).
"""

# ╔═╡ 19e90511-ca98-4a6a-81b2-47a9353b91fc
md"""
## Multivariate model
...
# ╔═╡ 43be5b14-81e7-49c5-9ea9-7086685e4828
model_2 = glm(@formula(Days ~ Eth + Sex + Age + Lrn), quine, Poisson())

# ╔═╡ ff592694-e9cd-4c18-b959-b781ed2be067
aov.anova(model_2; type=3)

# ╔═╡ c7e4147f-a4c8-4c73-9fc6-5c5e89c5e2c7
glm_coef(model_2 |> coeftable |> DataFrame)

# ╔═╡ 1fee5798-f113-494b-838e-594db3c9e691
md"""
### Goodness of fit
...
# ╔═╡ 8db4dfa5-b8c2-4a58-b886-de0f258600a5
deviance(model_2) / dof_residual(model_2) |> r3

# ╔═╡ 51e0d99c-810a-4ddc-94cf-fd442e187767
md"""
!!! warning \"Question\"
...
# ╔═╡ 695b5259-45c4-4227-bb68-0592606d40ed
md"""
!!! hint \"Answer\"

	There is overdispersion. The model does not properly fit the data.
"""

# ╔═╡ 72412906-10f4-42d1-b3d7-4ffc42698ac1
md"""
## Negative Binomial
...
# ╔═╡ c2d9b2d3-5a4f-48ce-a29e-226612990324
model_nb = glm(
...
# ╔═╡ add752ec-5593-4a85-9b2e-042e1f366115
aov.anova(model_nb, type=3)

# ╔═╡ 92daaf1b-b841-4c8d-9137-87b3bf148b91
md"""
!!! warning \"Question\"

	Did your results change? Which variables are no longer significant?
"""

# ╔═╡ df5a878f-3cc0-403c-b69d-e49a83be5196
md"""
!!! hint \"Answer\"

	Sex and type of learner are no longer significant.
"""

# ╔═╡ d0c2d88d-4e29-487b-83de-6f1fc12423e9
md"""
!!! note \"Exercise\"

	Remove not significant variables and run the model again.
"""

# ╔═╡ 5e26b7f1-efa9-414f-90a8-96d2c795ecf4
md"""
```julia
model2_nb = glm(
...
# ╔═╡ fc93d3d6-c490-4977-895d-3eb2b8547892
model2_nb = glm(
  @formula(Days ~ Eth + Age), 
  quine, NegativeBinomial(2.0),
  LogLink()
...
# ╔═╡ 785de0ca-7bc5-4cac-bc15-087f737d4294
md"""
...
# ╔═╡ 7a61181b-f6e0-43d9-8438-f2eed317f606
# ╠═╡ show_logs = false
aov.anova(model_nb, model2_nb)

# ╔═╡ 19e861ff-545f-4801-90b5-38214501c2d4
md"""
...
# ╔═╡ 8847e272-3004-4ccf-8fc0-c56041b8445f
md"""
### Table of coefficients
"""

# ╔═╡ ebc2a16e-1737-4e70-98ea-56af7c603c7e
glm_coef(model2_nb |> coeftable |> DataFrame)

# ╔═╡ ca90824b-7733-4d28-89c9-9e80090b0cb4
md"""
### Effect plot
"""

# ╔═╡ 502774f6-4729-4c99-ae73-2de89e8a1e69
eff_nb = effects(
  Dict(
...
# ╔═╡ fb44d4fa-f6f6-4958-8045-1f837c1cf412
@df eff_nb scatter(
	:Age, :Days, 
	group=:Eth, yerr=:err,
	xlab="Age group",
...
# ╔═╡ d259052a-eff6-4306-9a8e-72107645bdd8
md"""
# Incidence Rates
"""

# ╔═╡ 1c6f2f23-4eb4-4b46-a193-f1747b433071
md"""
!!! tip \"Example\"

	We are going to use the `seizures` data set. This data set contains information on the number of seizures experienced by patients with epilepsy. The data comes from a randomised controlled trial where patients were assigned to one of two treatments (new treatment or standard treatment) and there was variable amount of follow-up. The research question is: Can the new treatment reduce the rate of seizures in comparison to the standard treatment?
"""

# ╔═╡ f5de7880-d160-4a55-9ff6-8d8919eb1832
seizures = readr.read_rds("data/seizures.rds") |> rcopy;

# ╔═╡ 76b9253b-a9af-4b52-a3f0-a164af74aafb
seizures |> schema

# ╔═╡ 66711fd3-3dac-4cd1-a821-ceb2e048e43c
md"""
!!! note

	The follow-up time, `pweeks`, is included as an offset in the model, as we do not need a coefficient to be estimated. The back-transformed results become incidence rate ratios.
"""

# ╔═╡ 98c6d912-7f44-4e0b-9db6-1adac85fb69d
model_sei = glm(
...
# ╔═╡ 3c32b87e-cd70-498f-965b-d6c1252727ff
glm_coef(model_sei |> coeftable |> DataFrame)

# ╔═╡ d1868904-c017-4123-9845-4e5e7a0826f2
deviance(model_sei) / dof_residual(model_sei) |> r3

# ╔═╡ 339d0a4f-00bc-4fb5-8141-e717fe5381fc
md"""
!!! warning \"Question\"

	Do you think you may have overdispersion? Does your model fit the data? What can we do about it?
"""

# ╔═╡ f3b85608-e23e-4616-a102-e4cd9d0967e5
md"""
!!! hint \"Answer\"

	The deviance is about 6 times greater than the degrees of freedom from the residuals, which indicates overdispersion, indicting that our model is not appropriate for explaining the variability in the data. We can perform a negative binomial regression model to account for the overdispersion.
"""

# ╔═╡ 38c53edd-9860-485c-bf26-1b8b7fac18ef
model_snb = glm(
  @formula(nseize ~ treat),
  seizures, offset=log.(seizures.pweeks),
  NegativeBinomial(),
...
# ╔═╡ 5b3bf29b-316b-4bad-b4c7-c91f4eac866d
...
manifest_format = "2.0"
project_hash = "7464d9ba42c0bf0f3aac46e94958007e84cf010e"
